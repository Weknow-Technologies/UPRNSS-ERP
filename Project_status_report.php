					...
					<div class="row">
						...
						<a onclick="printPage()" class="no-print btn btn-info" style="color: white; background-color: #5cb85c; border-color: #4cae4c; padding: 5px 10px; border-radius: 4px; text-decoration: none;">Export to Excel</a>
							...
							<div class="card">
								...
								<h4 style="text-align:center; font-size:28px; margin:0px; ">&nbsp; परियोजनाओ की संख्या और उनका स्थिति यूनिट के आधार पर </h4>
								<div class="content">
			<!------------------------------Unit wise ----------------------------->						
									<div class="row">
										<table class="table table-striped table-bordered table-hover">
											...
											<tbody>
												...
												while($row_div = mysqli_fetch_assoc($result_div)){
													$sql = 'select * from uprnss_project_temp where division_id="'.$row_div['s_no'].'" and (status!="5" or status="0" or status is null or status="1") and (project_status_1="12")';
													$anubandh = mysqli_num_rows(execute_query($sql));
													
													$sql = 'select * from uprnss_project_temp where division_id="'.$row_div['s_no'].'" and (status!="5" or status="0" or status is null or status="1") and (project_status_1="5")';
													$unstarted = mysqli_num_rows(execute_query($sql));
													
													$sql = 'select * from uprnss_project_temp where division_id="'.$row_div['s_no'].'" and (status!="5" or status="0" or status is null or status="1") and (project_status_1="6")';
													$lack_of_money = mysqli_num_rows(execute_query($sql));
													
													$sql = 'select * from uprnss_project_temp where division_id="'.$row_div['s_no'].'" and (status!="5" or status="0" or status is null or status="1") and (project_status_1="4")';
													$controversial = mysqli_num_rows(execute_query($sql));
													
													$sql = 'select * from uprnss_project_temp where division_id="'.$row_div['s_no'].'" and (status!="5" or status="0" or status is null or status="1") and (project_status_1="10")';
													$progress = mysqli_num_rows(execute_query($sql));
													
													$sql = 'select * from uprnss_project_temp where division_id="'.$row_div['s_no'].'" and (status!="5" or status="0" or status is null or status="1") and (project_status_1="11")';
													$complete = mysqli_num_rows(execute_query($sql));
													
													$sql = 'select * from uprnss_project_temp where division_id="'.$row_div['s_no'].'" and (status!="5" or status="0" or status is null or status="1") and (project_status_1="7")';
													$inventory_sent = mysqli_num_rows(execute_query($sql));
													
													$sql = 'select * from uprnss_project_temp where division_id="'.$row_div['s_no'].'" and (status!="5" or status="0" or status is null or status="1") and (project_status_1="8")';
													...
													$sql = 'select * from uprnss_project_temp where division_id="'.$row_div['s_no'].'" and (status!="5" or status="0" or status is null or status="1") and (project_status_1="11" or project_status_1="7" or project_status_1="8")';
