...
if(isset($_POST['submit'])){
	if($_POST['edit_sno']==''){
		$sql = 'insert into master_project_type (project_type, status, created_by, creation_time)values ("'.$_POST['project_type'].'","0", "'.$_SESSION['username'].'", "'.date("Y-m-d H:i:s").'")';
		execute_query($sql);
		if(mysqli_error($db)){ 
			$msg .= '<p class="text text-danger">Error # 1 : '.mysqli_error($db).'>> '.$sql.'</p>';
		}
		if($msg==''){
			$msg .= '<p class="text text-success">Data Saave</p>';
			unset($_POST);
			goto postblank;
		}
	}
	else{
		$sql = 'update master_project_type set 
...
if(isset($_GET['edit_sno'])){
	$sql = 'select * from master_project_type where sno="'.$_GET['edit_sno'].'"';
...
if(isset($_GET['del'])){
	$sql = 'delete from master_project_type where sno="'.$_GET['del'].'"';
	execute_query($sql);
						...
						<div class="row" id="">
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="project_type" id="project_type" class="form-control" placeholder="" value="<?php echo $_POST['project_type']; ?>" tabindex="<?php echo $tab++; ?>">
                                </div>
                            </div>
                        </div>
						<div id="test"></div>
                ...
                <div class="card">
					...
					<table class="table table-striped table-hover table-bordered" id="general_stat_table">
						...
						<tbody>
							<?php
						$i=1;
						$sql = 'select * from master_project_type ';
