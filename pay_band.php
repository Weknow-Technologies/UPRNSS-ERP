...
if (isset($_POST['submit'])) {
	if ($_POST['edit_sno'] == '') {
		$sql = 'insert into master_pay_band (band_name, pay_scale, status, created_by, creation_time) values ("' . $_POST['band_name'] . '", "' . $_POST['pay_scale'] . '", "0", "' . $_SESSION['usersno'] . '", "' . date("Y-m-d H:i:s") . '")';
		execute_query($sql);
		if (mysqli_error($db)) {
		...
		$sql = 'update master_pay_band set 
...
if (isset($_GET['edit_sno'])) {
	$sql = 'select * from master_pay_band where sno="' . $_GET['edit_sno'] . '"';
	$data = mysqli_fetch_assoc(execute_query($sql));
	$_POST['edit_sno'] = $data['sno'];
	$_POST['band_name'] = $data['band_name'];
	$_POST['pay_scale'] = $data['pay_scale'];

}

if (isset($_GET['del'])) {
	$sql = 'delete from master_pay_band where sno="' . $_GET['del'] . '"';
	execute_query($sql);
					...
					<div class="row" id="">
						...
						<div class="col-md-3">
							<div class="form-group">
								...
								<input type="text" name="band_name" id="band_name" class="form-control" placeholder=""
									value="<?php echo $_POST['band_name']; ?>" tabindex="<?php echo $tab++; ?>">
							</div>
						</div>
					</div>
					<div id="test"></div>
		...
		<div class="card">
			...
			<div class="card-body">
				<table class="table table-striped table-hover">
					...
					<?php
					$i = 1;
					$sql = 'select * from master_pay_band ';
