...
if(isset($_GET['id'])){
	$sql = 'select * from master_zone where sno="'.$_GET['id'].'"';
	$zone = mysqli_fetch_assoc(execute_query($sql));
}
			...
			<div class="col-md-12">
				<div class="card">
					<div class="card-header">
                        <h4 class="card-title text-center"><?php echo $zone['zone_name']; ?> Summary Report</h4></br>
                    </div>
					<div class="content">
<!------------------------------Unit wise ----------------------------->						
						<div class="row">
							<table class="table table-striped table-bordered table-hover">
								...
								<tbody>
									<?php
									$sql = 'select * from uprnss_division where zone_id="'.$_GET['id'].'"';
									...
									while($row_div = mysqli_fetch_assoc($result_div)){
										...
										// $sql = 'SELECT s_no FROM uprnss_division WHERE zone_id="'.$row_div['sno'].'"';
										// $result_div = execute_query($sql);	
										// // echo $sql;
										// $sno_array = array();
										// while($row_div = mysqli_fetch_assoc($result_div)){
										...
										$sql = 'select count(*) c, sum(abs(sanction_cost)) as amount from uprnss_project_temp where division_id="'.$row_div['s_no'].'" and (status!="5" or status="0" or status is null or status="1") and (project_status_1="" or project_status_1 is null or project_status_1="3" or project_status_1="9" or project_status_1="5" or project_status_1="12" or project_status_1="13")';
