            ...
            <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title text-center bg-danger text-white p-2">निर्माण कार्य हेतु मुख्यालय से प्राप्त धनराशि का विवरण वर्ष 2023-24</h4></br>
                        </div>
                            <?php echo $msg; ?>
                        <div class="card-body">
                            <div class="row">
							...
							<div class="row border rounded m-2 p-2 border-secondary">
									...
									<select class="form-control" name="yojnaname_<?php echo $i; ?>" id="yojnaname_<?php echo $i; ?>" tabindex="<?php echo $tab++; ?>" onChange="fill_district(this.value), fill_sub_department(this.value)">
										...
										<?php
											$query = '(SELECT sno, project_name_hindi, Project_name FROM uprnss_project_temp WHERE division_id
											 IN (' . implode(",", $_SESSION['divisions']) . ') and status!="5" )';
											// echo $query;
								...
								<input onInput="addCalc()" type="text" class="form-control" name="gsttds_<?php echo $i; ?>" id="gsttds_<?php echo $i; ?>" value="<?php echo $_POST['gsttds_'.$i]; ?>"></div>
								
								<div class="col-md-3"><label>लेबर सेस</label>
								<input onInput="addCalc()" type="text" class="form-control" name="leboreses_<?php echo $i; ?>" id="leboreses_<?php echo $i; ?>" value="<?php echo $_POST['leboreses_'.$i]; ?>"></div>
								...
								<div class="form-group">
									...
									<?php 
										if (isset($_GET['edit_sno']) && $_GET['edit_sno'] != "") {
											...
											<a href="prarup_new_3.php" class="btn btn-info">Back</a>';
										}else{
					...
					<div class="card-body">
						<table class="table table-striped table-bordered table-hover">
							...
							if($_SESSION['usertype']=="sadmin"){
								...
								$sql = '(SELECT sno, "Manual" as type, division_id, entry_date, created_by  FROM `invoice_prarup_3`)
								union all 
								(SELECT sno, "Automatic" as type, unit_id as division_id, transafer_date as entry_date, created_by FROM `invoice_account_fund_transafer` where fund_transfer_to=3)
								 order by entry_date desc, division_id';
							}else{
								$sql = 'SELECT sno, "Manual" as type,  division_id, entry_date, created_by  FROM `invoice_prarup_3` where invoice_prarup_3.created_by = "'.$_SESSION['username'].'"';
							}
							...
							while($row_prarup_new_3 = mysqli_fetch_assoc($result_prarup_new_3)){
								...
								echo '<tr>
								<td>'.$i++.'</td>
								<td>'.$row_prarup_new_3['type'].'</td>
								<td>'.$row_div['division_name'].'</td>
								<td>'.$row_prarup_new_3['entry_date'].'</td>';
								
								if($row_prarup_new_3['type']=='Manual'){
									echo '<td><a href="'.$_SERVER["PHP_SELF"].'?delid='.$row_prarup_new_3['sno'].'" onClick="return confirm(\'Are you sure ?\');"><i class="far fa-trash-alt"></i></a></td>
...
<script>
		function addCalc(line_serial) {
			...
			for (let i = 1; i <= id; i++) {
				...
				document.getElementById("totalrupees_"+i).value=total_rashi_ans.toFixed(2);
				// console.log(total_rashi_ans);
				
				var awashesh_dhanrashi_ans=yojnalagat-total_rashi_ans;
				document.getElementById("avsheshdhanrashi_"+i).value=awashesh_dhanrashi_ans.toFixed(2);
