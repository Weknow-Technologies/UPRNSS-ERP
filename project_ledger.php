...
<div class="row">
    ...
    .fixed-header thead th {
        ...
        z-index: 2; /* Optional: Set z-index to keep header above table */
    }
	.table-container {
    width: 100%;
    overflow-x: auto;
...
.fixed-column {
    ...
    background-color: #fff;
						...
						<div class="card-body table-container" style="position:relative;">
							<table class="table table-striped table-hover table-bordered" id="general_stat_table"
								...
								<thead style="position:sticky;top:0; z-index:2;">
									<tr >
										...
										<th rowspan="2">परियोजना पर कुल व्यय </th>
								...
								where (status!="5" or status="0"  or status is null or status="1")';
										...
										<td><a href="report_ledger_project.php?id='.$row['sno'].'" target="_blank"><small>View Ledger</small></br>'.$row['project_name_hindi'].'</a></td>
										...
										<td>' . $row['admin_go_no'] . '</td>'; ?>
											...
											<?php echo '<td>' . $row['sanction_cost'] . '</td>'; ?>
...
table.addEventListener('scroll', function (e) {
    ...
    // Apply scroll to the first four fixed columns
    var fixedColumns = document.querySelectorAll('.fixed-column');
    for (var i = 0; i < fixedColumns.length; i++) {
        fixedColumns[i].style.transform = 'translateY(' + scrollTop + 'px)';
    }
});
		</script>
