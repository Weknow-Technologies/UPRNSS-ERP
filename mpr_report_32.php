	...
	<div id="container" class="no-print">
		...
		<div class="card card-body">
        	...
        	<div class="row d-flex my-auto">    	
					<table width="100%" class="table table-striped table-hover rounded">	
						...
						<tr >
							<th>Select Report View Mode</th>
							<th width="18%">
								<select name="report_view" id="report_view" class="form-control">
                    				<option value="1">Unit View</option>
                    				<option value="2">Department View</option>
                    			</select>
							</th>
							<th>प्रखण्ड </th>
							<th width="18%"><select class="form-control" name="division" id="division" tabindex="<?php echo $tab++; ?>">
									<option value="">--- Select ---</option>
									<?php
									$query = 'select * from uprnss_division where s_no in ('.implode(",", $_SESSION['divisions']).') ORDER BY division_name ASC';
								// echo $query;
									$run = mysqli_query($db,$query);
									while($data = mysqli_fetch_array($run)){
										echo '<option value="'.$data['s_no'].'" ';
										if(isset($_POST['division'])){
											if($_POST['division']==$data['s_no']){
					...
					<div class="mx-auto">
						<button type="submit" class="btn btn-primary" name="generate_report" id="generate_report">Generate Report</button>						
					</div>
        ...
        <div class="row">
			...
			<!-- <button onclick="printTable()">Print Table</button> -->
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title text-center">Monthly Progress Report </h4></br>
                    </div>
						...
						if(isset($_POST['report_view'])){
							switch($_POST['report_view']){
								case '1':{
									...
									invoice_civil.remark as remark
									from invoice_civil 
									...
									while($row_project = mysqli_fetch_assoc($result_div)){
										if($div == ''){
											...
											<thead>
												<tr>
													...
													<th rowspan="2">कुल प्राप्त धनराशि व दिनांक </th>
													<th rowspan="2">कुल प्राप्त धनराशि</th>
													<th colspan="3">कुल व्यय धनराशि </th>
													<th rowspan="2">अवशेष धनराशि (परियोजना लागत के सापेक्ष)<br>(20-23)</th>
													<th rowspan="2">वित्तीय प्रगति %(अवमुक्त के सापेक्ष)</th>
													<th rowspan="2">संबन्धित विभाग से अप्राप्त धनराशि <br>(12-20)</th>
												...
												<tr>
													<th>परियोजना परवर्तमान माह में व्यय </th>
										...
										<td>'.$row_project['admin_go_date'].'</td>
										<td>'.$row_project['land_receive_date'].'</td>
										<td>'.$row_project['work_start_date'].'</td>
										<td>'.$row_project['work_completion_date'].'</td>
										...
										$final_txt .='<td>'.$row_project['revised_date'].' '.$row_project['revised_cost'].'</td>';
										$final_txt .= '<td>&nbsp;</td>';
										$final_txt .= $act_txt;
										$final_txt .= '<td>&nbsp;</td>';
										$final_txt .= '<td>&nbsp;</td>';
								...
								case '2':{
									...
									while($row_project = mysqli_fetch_assoc($result_depart)){
										...
										$final_txt .= '<td>'.$row_project['balance_amt_project'].'</td>';
										$final_txt .= '<td>'.$row_project['balance_amt_cost'].'</td>';
