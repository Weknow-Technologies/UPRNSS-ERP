	...
	<div id="container" class="no-print">	
		<div class="card card-body">    
        	...
        	<div class="row d-flex my-auto">    	
					...
					<?php $select_array = array("chk_department_name_hindi" => "विभाग", "chk_project_name_hindi" => "कार्य  का नाम (हिन्दी )","chk_financial_go_date" => "वित्तीय स्वीकृति की तिथि", "chk_sacntion_cost" => "कुल  लागत ( लाख में )","chk_balance_amount_refer" => "अवमुक्त की गई धनराशि ( लाख में )", "chk_project_status_1" => "परियोजना की स्थिति  1", "chk_remarks" => "अभियुक्ति" );
					?>
                    <table class="table table-border table-striped d-none">
                        <?php
                        $i=1;
					...
					<a href="report_custom_export.php" style="color: #ffffff"><button type="button" name="student_ledger" class="btn btn-danger" >Download In Excel</button></a>
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                     <?php if(isset($_POST['search'])){?>   <h4 class="card-title text-center"> दिनांक-(<?php echo date("d-m-Y" ,strtotime($_POST['date_from'])); ?>) से दिनांक- (<?php echo date("d-m-Y" ,strtotime($_POST['date_to'])); ?>)  तक आवंटित कार्य एवं उसकी प्रगति</h4><?php };?></br>
                     <div class="row">
                     	<div class="col-3">
                     		<input type="text" name="project_name_h" id="project_name_h" placeholder="Project Name" class="form-control">		
                     	</div>
						<div class="col-3">
                     		<input type="text" name="go_number" id="go_number" placeholder="GO Number" class="form-control">		
                     	</div>
                     	<div class="col-3">
                     		<input type="date" name="go_date" id="go_date" placeholder="GO Date" class="form-control">		
                     	</div>
                     	<div class="col-3">
                     		<button type="submit" name="update_go" id="update_go" class="btn btn-warning">Update GO Information</button>
                     	</div>
					...
					<table class="table table-striped table-hover table-bordered" id="general_stat_table">
						...
						<th class="text-center">GO. date</th>
						<th class="text-center">Project cost</th>
						<th class="text-center">Release Amount</th>
						...
						<th class="no-print text-center">InvoiceNo.</th>
						<th class="no-print text-center">TransNo.</th>
						...
						<tr>
							...
							<th>10</th>
							<th>11</th>
							<th class="no-print">12</th>
							<th class="no-print">13</th>
							<th class="no-print">14</th>
							<?php
							$i=2;
							foreach($select_array as $k=>$v){
								if(isset($_POST[$k])){
						...
						//if($_SESSION['usertype']=='sadmin'){
							...
							financial_go_amount,
							uprnss_project_temp.district_id as district_id,
							uprnss_project_temp.division_id as division_id,
							department_id,
							sub_department_id,
							new_project_id,
							new_project_trans_id,
							...
							if(isset($_POST['search'])){
								...
								//$todate=date("Y-m-d");
								//$fromdate = date("Y-m-d", strtotime($todate.'-2 Years'));

								$sql .= ' and '.$_POST['date_type'].'>="'.$_POST['date_from'].'" and '.$_POST['date_type'].'<"'.$_POST['date_to'].'"';
							...
							while($row = mysqli_fetch_assoc($result)){
								...
								if($row['new_project_id']=='' && isset($_POST['update_go']) && isset($_POST['project_'.$row['sno']])){
									//echo 'test<br>';
									if($first==0){
										$first = 1;
										$sql = 'INSERT INTO `invoice_new_project` (`department`, `sub_department_id`, `go_type`, `go_number`, `go_date`, `go_short_number`, `sanction_cost`, `sanction_date`, `central_share`, `state_share`, `project_name_english`, `project_name_hindi`, `scheme`, `sub_scheme`, `project_under`, `status`, `created_by`, `creation_time`) VALUES ("'.$row['department_id'].'", "'.$row['sub_department_id'].'", "1", "'.$_POST['go_number'].'", "'.$_POST['go_date'].'", "NA", "'.$row['revised_estimate_amount'].'", "NA", "NA", "NA", "'.$_POST['project_name_h'].'", "'.$_POST['project_name_h'].'", "NA", "NA", "NA", 0, "'.$_SESSION['username'].'", "'.date("Y-m-d H:i:s").'")';
										execute_query($sql);
										if(mysqli_error($db)){
											echo 'Error # 1.0.03 : '.mysqli_error($db).' >> '.$sql;
										}
										else{
											$go_insert_id=mysqli_insert_id($db);	
											$sql = 'INSERT INTO `transaction_new_project` ( `invoice_id`, `division_id`, `district_id`, `sub_project_name`, `sub_project_name_hindi`, `status`, `created_by`, `creation_time`,`civil_status`) VALUES ("'.$go_insert_id.'", "'.$row['division_id'].'", "'.$row['district_id'].'", "'.$row['project_name'].'", "'.$row['project_name_hindi'].'", 0, "'.$_SESSION['username'].'", "'.date("Y-m-d H:i:s").'", "0")';
											execute_query($sql);
										}
									...
									$sql = 'update uprnss_project_temp set new_project_id="'.$go_insert_id.'", new_project_trans_id="'.mysqli_insert_id($db).'", admin_go_no="'.$_POST['go_number'].'", admin_go_date="'.$_POST['go_date'].'" where sno="'.$row['sno'].'"';
									execute_query($sql);
								}
								
								
								if($row['new_project_id']==''){
									echo '<tr onClick=\'$("#project_'.$row['sno'].'").prop("checked", !$("#project_'.$row['sno'].'").prop("checked"));\'>
									<td class="no-print"><input type="checkbox" name="project_'.$row['sno'].'" id="project_'.$row['sno'].'" value="'.$row['sno'].'"></td>';
								}
								else{
									echo '<tr><td></td>';
								}
								...
								if(isset($_POST['chk_financial_go_date'])){
								    echo '<td>';
								...
								echo '<td class="no-print">'.$row['new_project_id'].'</td>';
								echo '<td class="no-print"> '.$row['new_project_trans_id'].'</td>';
