...
?>
        ...
        ?>
		<form name="form_filters" id="form_filters" enctype="multipart/form-data" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
		<table class=" table table-striped table-hover table-bordered">
       		<tr>
				<th>Name</th>
				<td><input type="text" name="cust_name" tabindex="<?php echo $tab++; ?>" id="cust_name" value="<?php if(isset($_SESSION['report_ledger_cust_name'])){echo $_SESSION['report_ledger_cust_name'];} ?>" class="field text medium" />
				<input type="hidden" id="comment"></td>
				<th>Date From</th>
				<td><script type="text/javascript" language="javascript">
				document.writeln(DateInput('date_from', 'filters', false, 'YYYY-MM-DD', '<?php if(isset($_SESSION['report_ledger_date_from'])){echo $_SESSION['report_ledger_date_from'];}else{echo date("Y-m-d");} ?>', '<?php echo $tab++; $tab+=3; ?>'));
				</script><input type="hidden" name="custsno" id="custsno"></td>
				<th>Date To</th>
				<td colspan="2"><script type="text/javascript" language="javascript">
				document.writeln(DateInput('date_to', 'filters', false, 'YYYY-MM-DD', '<?php if(isset($_SESSION['report_ledger_date_to'])){echo $_SESSION['report_ledger_date_to'];}else{echo date("Y-m-d");} ?>', '<?php echo $tab++; $tab+=3; ?>'));
				</script><input type="hidden" name="cust_sno" id="cust_sno"></td>
             </tr>
             <tr>
             	<th>Parent</th>
             	<th><select name="parent" tabindex="<?php echo $tab++; ?>">
                    <option></option>
                    <option value="NA" <?php if(isset($_POST['parent'])){if($_POST['parent']=='NA'){echo ' selected="selected"';}}?>>No Parent</option>
                    <?php 
             	...
             	<th>Mobile</th>
             	<th><input type="text" name="mobile" tabindex="<?php echo $tab++; ?>" id="mobile" value="<?php if(isset($_SESSION['report_ledger_mobile'])){echo $_SESSION['report_ledger_mobile'];} ?>" class="field text medium" /></th>
             	<th>Amount</th>
				<td colspan="2">
				<select name="amount_symbol" id="amount_symbol" tabindex="<?php echo $tab++; ?>">
					<option value="=" <?php if(isset($_SESSION['report_ledger_amount_symbol'])){if($_SESSION['report_ledger_amount_symbol']=='='){echo 'selected';}}?>>=</option>
					<option value=">=" <?php if(isset($_SESSION['report_ledger_amount_symbol'])){if($_SESSION['report_ledger_amount_symbol']=='>='){echo 'selected';}}?>>>=</option>
					<option value="<=" <?php if(isset($_SESSION['report_ledger_amount_symbol'])){if($_SESSION['report_ledger_amount_symbol']=='<='){echo 'selected';}}?>><=</option>
				</select>
				<input id="amount" name="amount" class="fieldtextmedium" maxlength="255" tabindex="<?php echo $tab++; ?>" type="text" value="<?php if(isset($_SESSION['report_ledger_amount'])){echo $_SESSION['report_ledger_amount'];}?>">
				</td>
             </tr>
             <tr>
             	<th>Type</th>
    ...
    <table class=" table table-striped table-hover table-bordered" id="ledgerReportTable">
		...
		for ($pgid = $start; $pgid < $end; $pgid++) {
			...
			if($row['type']=='stock'){
				...
				$chk_amount = get_cust_balace($from,$to,$row['sno']);
			}
			...
			if($row['type']=='stock'){
				...
				if($row['type']=='billit_customer' && $row['auto_ledger']==''){
					$opening = get_cust_balace('1970-01-01',$from,$row['sno']);
					$closing = get_cust_balace($from,$to,$row['sno']);
...
?>
		...
		<div class="col-md-12">
			<div class="topnav no-print">
				<a href="billit_customer_ledger.php?details=1&ledid=<?php echo $_GET['ledid'].'&df='.$_GET['df'].'&dt='.$_GET['dt'];?>"><span class="glyphicon glyphicon-file"></span>Detailed Ledger</a>&nbsp;|&nbsp;
				<a href="billit_customer_ledger_stock.php?ledid=<?php echo $_GET['ledid'].'&df='.$_GET['df'].'&dt='.$_GET['dt'];?>"><span class="glyphicon glyphicon-file"></span>Stock Ledger</a>
			</div>
        ...
        <tr><td colspan="6">
            ...
            Ledger From :<?php if(isset($_GET['df'])){ echo $_GET['df']; }?> to <?php if(isset($_GET['dt'])){ echo $_GET['dt']; }?><br/>
            ...
            <form action="<?php echo $_SERVER['REQUEST_URI'];?>" method="post" enctype="multipart/form-data">
            	<table class=" table table-striped table-hover table-bordered">
            		<tr>
            			...
            			<td><input type="checkbox" name="filter_debit" id="filter_debit" <?php if(isset($_POST['filter_debit'])){echo ' checked="checked"';}elseif(!isset($_POST['filter_search'])){echo ' checked="checked"';}?> > Payments</td>
            			<td><input type="checkbox" name="filter_credit" id="filter_credit" <?php if(isset($_POST['filter_credit'])){echo ' checked="checked"';}elseif(!isset($_POST['filter_search'])){echo ' checked="checked"';}?>> Receipts</td>
            			<td><input type="submit" name="filter_search" id="filter_search" value="Search"></td>
            		</tr>
            	</table>
            ...
            <div id="party" style="font-size:18px;">
                ...
                ?>
                <?php echo 'CURRENT BALANCE: '.amount_format(get_cust_balace($_GET['df'],$_GET['dt'],$_GET['ledid']));?><br />
            </div>
		</td>
		</tr>
        <tr>
...
<?php
		if(!isset($_GET['t'])){
			if(isset($_GET['df']) && isset($_GET['dt'])){
				...
				//echo $sql;
				$billit_customer['opening_balance'] = get_cust_balace("1970-01-01", date("Y-m-d", strtotime($_GET['df'])-86400),$billit_customer['sno']);
			}
		}
		elseif(isset($_GET['t'])){
			if($_GET['i_o']=='i'){
