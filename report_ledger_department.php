...
if(isset($_GET['id'])){
	...
	$row_department = mysqli_fetch_assoc($division_result);
						...
						<div class="row">
							<div class="col-md-3">
								<div class="form-group">
									...
									<?php echo $row_department['department_name_hindi']; ?>
								</div>
							</div>
						</div>						
					</div>
			...
			<div class="card strpied-tabled-with-hover">
				...
				<div class="card-body table-full-width table-responsive">
					...
					<table class="table table-hover table-striped table-bordered">
						...
						<tbody>
							<?php
							$sql = '(SELECT transaction_fund_receive.sno as sno, p_receive_amount, fund_receive_to, installment, order_no, order_date, receive_date, request_no, request_date, bank_name, account_no, ifsc_code, "receive" as type FROM `transaction_fund_receive` left join invoice_fund_receive on invoice_fund_receive.sno = invoice_id where transaction_fund_receive.department_id="'.$_GET['id'].'" order by receive_date) 
							union all 
							(SELECT invoice_account_fund_transafer.sno as sno, praposemoney, fund_transfer_to, "" as installment, order_no, order_date, transafer_date, "" as request_no, "" as request_date, to_bank_name, to_account_no, to_bank_ifsc, "transfer" as type FROM `invoice_account_fund_transafer` where department="'.$_GET['id'].'")';
							//echo $sql;
							...
							while($row = mysqli_fetch_assoc($result)){	
								...
								$tot+= (float)$row['p_receive_amount'];
