...
if(isset($_POST['submit'])){
	if($_POST['edit_sno']==''){
		$sql = 'insert into dp_category (category_name) values ("'.$_POST['category_name'].'")';
		execute_query($sql);
		if(mysqli_error($db)){ 
...
if(isset($_GET['edit_sno'])){
	$sql = 'select * from dp_category where sno="'.$_GET['edit_sno'].'"';
	$data = mysqli_fetch_assoc(execute_query($sql));
	$_POST['edit_sno'] = $data['sno'];
	$_POST['category_name'] = $data['category_name'];
}	

if(isset($_GET['del'])){
	$sql = 'delete from dp_category where sno="'.$_GET['del'].'"';
	execute_query($sql);
						...
						<div class="row" id="">
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="category_name" id="category_name" class="form-control" placeholder="" value="<?php echo $_POST['category_name']; ?>" tabindex="<?php echo $tab++; ?>">
                                </div>
                            </div>
                        </div>
						<div id="test"></div>
