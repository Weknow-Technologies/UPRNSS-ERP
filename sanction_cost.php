            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title text-center"><?php echo $department['department_name_hindi']; ?></h4></br>
                    </div>
					<?php echo $msg; ?>
                    <div class="card-body">
						<table class="table table-hover table-striped table-bordered">
							<thead>
								<tr>
									...
									<th rowspan="2">Sanction Cost</th>
										
							</thead>
							<tbody>
							<?php
							$sql = 'SELECT uprnss_project_temp.sno as sno, project_name_hindi, master_freeze, sanction_cost, department_name_hindi, division_name, district_name_hindi, admin_go_no  FROM `uprnss_project_temp` 
							...
							where department_id in ('.$_GET['id'].') and (status!="5" or status="0" or status is null or status="1") and (project_status_1!="8" or project_status_1!="11" ) ORDER BY `uprnss_division`.`division_name` ASC ';
							$result = execute_query($sql);
							$i=1;
							while($row = mysqli_fetch_assoc($result)){
								$sql = 'SELECT * FROM `invoice_civil` where project_name="'.$row['sno'].'"  and last_update>="'.date("Y-m-01").'" and last_update<="'.date("Y-m-04").'"';
								...
								$sql = 'SELECT * FROM `invoice_civil` where project_name="'.$row['sno'].'"  and last_update>="'.date("Y-m-04").'" and last_update<="'.date("Y-m-t").'"';
								//echo $sql.'<br>';
