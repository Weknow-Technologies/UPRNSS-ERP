		...
		<div class="card strpied-tabled-with-hover">
			...
			<div class="card-body table-full-width table-responsive">
				...
				<table class="table table-hover table-striped table-bordered">
					...
					<tbody>
						<?php
						$sql = 'SELECT * FROM `billit_invoice_journal` where sno="' . $_GET['id'] . '" ORDER BY sno desc';
						...
						while ($row = mysqli_fetch_assoc($result)) {
							echo '<tr>
                                ...
                                <td>' . $row['first_to'] . '</td>
                                <td>' . $row['tot_debit'] . '</td>
								<td class="no-print"><a href="billit_journal_print.php?id=' . $row['sno'] . '" target="_blank">View</a></td>
								<td class="no-print text-center">
									<a href="billit_journal.php?id=' . $row['sno'] . '" target="_blank"><span class="far fa-edit" aria-hidden="true" data-toggle="tooltip" title="Edit Data"></span></a>
								</td>
								<td class="no-print text-center">
									<a href="billit_journal.php?del=' . $row['sno'] . '" onClick="return confirm(\'Are you sure?\');" style="color:#f00"><span class="far fa-trash-alt" aria-hidden="true" data-toggle="tooltip" title="Delete"></span></a>
								</td>
								</tr>';
						}
						...
						if (mysqli_num_rows($result_act) != 0) {
							...
							while ($row_transaction = mysqli_fetch_assoc($result_act)) {
								$by_amount = "";
								$to_amount = "";
								if ($row_transaction['by'] != "") {
									$ledger = get_ledger($row_transaction['by']);
									$by_amount = $row_transaction['amount'];
								} else {
									$ledger = get_ledger($row_transaction['to']);
									$to_amount = $row_transaction['amount'];
								}

								$act_txt .= '<tr>
                                        <td>' . $j++ . '</td>
										<td>' . $ledger . '</td>
										<td style="text-align:right;">' . $by_amount . '</td>
										<td style="text-align:right;">' . $to_amount . '</td>
										<td>' . $row_transaction['remarks'] . '</td>
									</tr>';
							}
						}
						$act_txt .= '</table></td></tr>';
