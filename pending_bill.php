        ...
        if (empty($suffixes)) {
            ...
            foreach ($suffixes as $suf) {
                ...
                if (isset($_FILES[$photo1_key]) && isset($_FILES[$photo1_key]['error']) && $_FILES[$photo1_key]['error'] == 0) {
                    ...
                    if ($_FILES[$photo1_key]['size'] <= 2 * 1024 * 1024) {
                        ...
                        if (in_array($mime_type, $allowed_types)) {
                            ...
                            $errors[] = 'Invalid file type for Photo 1 (Bill ' . htmlspecialchars($bill_no) . ').';
                        }
                    } else {
                ...
                if (isset($_FILES[$photo2_key]) && isset($_FILES[$photo2_key]['error']) && $_FILES[$photo2_key]['error'] == 0) {
                    ...
                    if (in_array($photo2_ext, ['jpg', 'jpeg', 'png']) && $_FILES[$photo2_key]['size'] <= 2 * 1024 * 1024) {
                        $photo2 = $upload_dir . 'photo2_' . time() . '_' . uniqid() . '.' . $photo2_ext;
                        move_uploaded_file($_FILES[$photo2_key]['tmp_name'], $photo2);
                    } else {
                        $errors[] = 'Photo 2 must be JPG/PNG and less than 2MB (Bill ' . htmlspecialchars($bill_no) . ').';
                    }
                }
...
<style>
    ...
    .card-like{background:#fff; border:1px solid #e0e0e0; border-radius:6px; padding:15px; box-shadow:0 2px 6px rgba(0,0,0,0.08); margin-bottom:15px;}
    .bill-inner{border:2px solid #000; border-radius:6px; padding:15px; margin:0;}
    .bill-inner label{font-size:12px; margin-bottom:4px;}
    .bill-inner .form-group{margin-bottom:14px;}
    .bill-inner .compact [class^='col-']{padding-left:10px; padding-right:10px;}
</style>
...
?>
	...
	<div id="container" class="no-print">
		...
		<div class="card card-body">
			<div class="d-flex justify-content-between align-items-right mb-3">
					...
					<a href="pending_bill.php?view=view" style="color: #ffffff"><button type="button" name="student_ledger" class="btn btn-danger" >Click Here for Paid bill List</button></a>
				</div>
			<?php
                if($msg != ''){
							...
							if(isset($_POST['search'])){
								// if($_SESSION['usertype']=='11' || $_SESSION['usertype']=='3' || $_SESSION['usertype']=='5' || $_SESSION['usertype']=='4'){
									...
									while ($row = mysqli_fetch_assoc($result)) {
										...
										echo '<td>' . $i++ . '</td>';
										echo '<td>';
										if ($row['project_type'] == '1') echo 'शासन स्तर';
										elseif ($row['project_type'] == '2') echo 'जिला स्तर';
										echo '</td>';
	   ...
	   <form id="sale_form" name="sale_form" class="" autocomplete="off" enctype="multipart/form-data" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" onSubmit="">
			<div class="card">
				...
				<div class="card-body">
					...
					<div class="row" style="background:#f9f9f9; padding:10px; border-radius:5px; margin-bottom:15px; border:1px solid #ddd;">
						<div class="col-md-4">
							<div class="form-group">
								<label>Enter ERP Code / Tender No.</label>
								<input type="text" id="erp_tender_code" class="form-control" placeholder="ERP Code or Tender No">
							</div>
						</div>
						<div class="col-md-2">
							<div class="form-group">
								<label>&nbsp;</label><br>
								<button type="button" class="btn btn-info btn-fill" onclick="fetchProjectDetails()">Fetch Detail</button>
							</div>
						</div>
					</div>
					<div class="row">
						<div class="col-md-3 ">
							...
							<select class="form-control" name="department" id="department" tabindex="<?php echo $tab++; ?>" onChange="fill_sub_department(this.value); fill_district(this.value);">
									<option value="">--- Select ---</option>
									<?php
									$query = "select * from uprnss_department_name";
									$run = mysqli_query($db,$query);
                        ...
                        <div class="card-like">
							<button type="button" id="addBillRow" class="btn btn-primary" style="padding:8px 16px; font-size:14px; margin-left:90%;">
                        ...
                        <div class="row compact" style="width:100%; margin:0;">
                             <div class="col-md-3">
                                  <div class="form-group">
                                      <label>Bill No.</label>
                                      <input type="text" name="bill_no" id="bill_no" class="form-control" placeholder="" value="<?php echo $_POST['bill_no']; ?>" tabindex="<?php echo $tab++; ?>">
                                  </div>
                              </div>
                             <div class="col-md-3">
                                  <div class="form-group">
                                      <label>Bill Date</label>
                                      <input type="date" name="bill_date" id="bill_date" class="form-control" value="<?php echo htmlspecialchars($_POST['bill_date']); ?>">
                                  </div>
                              </div>
                             <div class="col-md-3">
                                  <div class="form-group">
                             ...
                             <div class="col-md-3">
                                  <div class="form-group">
                                      <label>Remark</label>
                                      <textarea type="text" name="unit_remark" id="unit_remark" class="form-control" placeholder="" tabindex="<?php echo $tab++; ?>"> <?php echo $_POST['unit_remark']; ?> </textarea>
                                  </div>
                              </div>
                        </div>
                        <div class="row compact" style="width:100%; margin:0;">
                             <div class="col-md-6">
                                  <div class="form-group">
                                      ...
                                      <input type="file" name="photo1" id="photo1" class="form-control" accept="image/*" onchange="validateAndPreview(this, 'preview1')" required>
                                      <img id="preview1" src="#" alt="Preview Image 1" style="display:none; width:100px; margin-top:10px; border:1px solid #ccc;"/>
                                  </div>
                              </div>
                             <div class="col-md-6">
                                  <div class="form-group">
                                      <label>Upload Photo 2</label>
                                      <input type="file" name="photo2" id="photo2" class="form-control" accept="image/*" onchange="validateAndPreview(this, 'preview2')">
                                      <img id="preview2" src="#" alt="Preview Image 2" style="display:none; width:100px; margin-top:10px; border:1px solid #ccc;"/>
                                  </div>
                              </div>
                        </div>
                        </div>
                    ...
                    <div class="row">
						<div class="col-md-12"  align = "center">
								<div class="form-group">
								<button type="submit" name="submit" class="btn btn-success btn-fill pull-right">Genrate Bill</button>
								<input type="hidden" id="edit_sno" name="edit_sno" value="<?php echo $_POST['edit_sno']; ?>">
								</div>
						</div>
					</div>
	...
	?>
	<div id="container" class="no-print">
		<form id="sale_form" name="sale_form" class="" autocomplete="off" enctype="multipart/form-data" method="post" action="pending_bill.php?view=view" onSubmit="">
		<div class="card card-body">    
        	<div class="row d-flex my-auto">  
						<div class="col-md-12 text-right" ><a href="pending_bill.php" style="color: #ffffff"><button type="button" name="student_ledger" class="btn btn-danger" >Pending For Payment bill list</button></a></div>
					<table width="100%" class="table table-striped table-hover rounded">	
						<tr >
							<th>विभाग </th>							
							<th width="18%">
		...
		<div class="row">
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
					<div class="col-md-12 text-center">
							<button  formtarget="_blank" type="submit" name="note_sheet" class="btn btn-primary text-right"  >Print Note Sheet</button>
						</div>
					<table class="table table-striped table-hover table-bordered" id="">
						<thead style="position:sticky;top:0; z-index:2;">
							<tr>
...
(function(){
    ...
    addBtn.addEventListener('click', function(){
        ...
        '<div class="col-md-6">'+
            '<div class="form-group">'+
                '<label>Upload Photo 1 *</label>'+
                '<input type="file" name="photo1_'+counter+'" id="photo1_'+counter+'" class="form-control" accept="image/*" onchange="validateAndPreview(this, \"preview1_'+counter+'\")">'+
        ...
        '<div class="col-md-6">'+
            '<div class="form-group">'+
                '<label>Upload Photo 2</label>'+
                '<input type="file" name="photo2_'+counter+'" id="photo2_'+counter+'" class="form-control" accept="image/*" onchange="validateAndPreview(this, \"preview2_'+counter+'\")">'+
