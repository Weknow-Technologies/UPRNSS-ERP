					...
					<div class="card strpied-tabled-with-hover">
						<div class="card-header text-right">
							<div class="ml-auto no-print">
								<a href="architect_alloted_report.php" class="text-right" target=""><u><i class="icon fas fa-file-alt" aria-hidden="true"></i> Alloted Report</u></a>
							</div>
						</div>
									...
									<?php
										...
										$sql = "SELECT *
											...
											WHERE status != '5' AND status != '2'
											  AND NOT EXISTS (
												  SELECT 1
												  FROM `inovoice_architect_allotment`
												  WHERE `inovoice_architect_allotment`.`project_id` = `uprnss_project_temp`.`sno` and inovoice_architect_allotment.status!='5'
											  )
											ORDER BY sno DESC";
										// echo $sql;
										...
										while($row = mysqli_fetch_assoc($result)){	
												...
												<td>'.$row['admin_go_no'].'<br>';if ($row['admin_go_date'] == "") {echo "-";  } else {echo date("d-m-Y", strtotime($row['admin_go_date']));};echo'</td>
												<td>'.$district['district_name_hindi'].'</td>
												<td>'.$divisions['division_name'].'</td>
												
												<td class="no-print text-center">
													<a href="architect_namit.php?id='.$row['sno'].'" onClick="return confirm(\'Are you sure you?\');">Allot<span class="far fa-edit" aria-hidden="true" data-toggle="tooltip" title=""></span></a>
												</td>
