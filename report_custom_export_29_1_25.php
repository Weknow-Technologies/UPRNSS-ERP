						...
						//if($_SESSION['usertype']=='sadmin'){
							...
							while($row = mysqli_fetch_assoc($result)){
								...
								if(isset($_POST['chk_sacntion_cost'])){
									...
									$html .= '<td>'.$row['sanction_cost'].'</td>';
								}
								if(isset($_POST['chk_revised_date'])){
								    $html .= '<td>'.$row['revised_date'].'</td>';
								}
								if(isset($_POST['chk_revised_cost'])){
								    $html .= '<td>'.$row['revised_cost'].'</td>';
								}
								...
								if(isset($_POST['chk_revised_amount'])){
								    $html .= '<td>'.$row['revised_estimate_amount'].'</td>';
								}
								if(isset($_POST['chk_revision_reason'])){
								    $html .= '<td>'.$row['revised_estimate_remark'].'</td>';
								}
								if(isset($_POST['chk_revision_date'])){
								    $html .= '<td>'.$row['revised_estimate_date'].'</td>';
								}
								if(isset($_POST['chk_revision_sent_status'])){
								    $html .= '<td>'.$row['revised_dispatch_status'].'</br>'.$row['revised_remittance_amount'].'</td>';
								}
								if(isset($_POST['chk_revision_sent_date'])){
								    $html .= '<td>'.$row['revised_remittance_date'].'</td>';
								}
								...
								if(isset($_POST['chk_percent_avmukt'])){
									$html .='<td>';
											if($row_invoice['total_received_amount']==0){
												...
												$html .= round($row_invoice['total_received_amount']/$row['sanction_cost']*100);
											}
