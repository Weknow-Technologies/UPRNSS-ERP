		...
		<div class="card strpied-tabled-with-hover">
			<div class="card-header text-right">
				<div class="ml-auto no-print">
					<a href="tender_alloted_report.php" class="text-right" target=""><u><i
								class="icon fas fa-file-alt" aria-hidden="true"></i> Alloted Report</u></a>
				</div>
			</div>
			<div class="card-header text-center bg-danger">
				<h3 class="text-white">निविदा हेतु शेष परियोजनाएँ</h3>
			</div>
					...
					<tbody>
						...
						WHERE status != '5' AND status != '2'
							AND NOT EXISTS (
								...
								WHERE `tender_allotment`.`project_id` = `uprnss_project_temp`.`sno` and tender_allotment.status!='5'
							)
						ORDER BY sno DESC";
						// echo $sql;
						...
						while ($row = mysqli_fetch_assoc($result)) {
							...
							<td class="no-print text-center">
								<a href="create_tender.php?id=' . $row['sno'] . '" onClick="return confirm(\'Are you sure you?\');">Allot<span class="far fa-edit" aria-hidden="true" data-toggle="tooltip" title=""></span></a>
							</td>
