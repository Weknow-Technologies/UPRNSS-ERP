...
if(isset($_POST['submit'])){
	if($_POST['edit_sno']==''){
		$sql = 'insert into uprnss_project_scheme (department_id, sub_department_id, scheme_name_english,scheme_name_hindi, status, created_by, creation_time) values ("'.$_POST['department'].'", "'.$_POST['sub_department_id'].'", "'.$_POST['scheme_name_english'].'","'.$_POST['scheme_name_hindi'].'","0", "'.$_SESSION['username'].'", "'.date("Y-m-d H:i:s").'")';
		execute_query($sql);
		if(mysqli_error($db)){ 
