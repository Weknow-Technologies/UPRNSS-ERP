...
if (isset($_GET['ajax']) && $_GET['ajax'] == '1') {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        // Balance lookup by sno directly (it's unique)
        $b = mysqli_query($db, "SELECT opening_balance FROM billit_customer WHERE sno=$sno LIMIT 1");
        $br = ($b && mysqli_num_rows($b) > 0) ? mysqli_fetch_assoc($b) : null;
        $ownBal = $br ? (float) $br['opening_balance'] : 0.0;
...
function get_descendant_balance($db, $headSno, $parentLedgerSno, $selected_division)
{
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        $b = mysqli_query($db, "SELECT opening_balance FROM billit_customer WHERE sno=$sno LIMIT 1");
        $br = ($b && mysqli_num_rows($b) > 0) ? mysqli_fetch_assoc($b) : null;
        $total += $br ? (float) $br['opening_balance'] : 0.0;
...
function fetch_heads_by_type($db, $fund_type, $selected_division)
{
    ...
    foreach ($heads as $h) {
        ...
        <div class="ledger-node">
            <div class="ledger-card head-card clickable"
                 onclick="toggleNode(\'h-' . $sno . '\',' . $sno . ',\'head\',' . $sno . ')">
        ...
        fetch(url)
            ...
            .then(function (data) {
                ...
                data.forEach(function (row) {
                    ...
                    if (hasCh) {
                        if (isPlHead) {
                            html += ' onclick="toggleNode(\'' + nKey + '\',' + row.sno + ',\'head\',' + row.sno + ')"';
                        } else {
                            html += ' onclick="toggleNode(\'' + nKey + '\',' + row.sno + ',\'customer\',' + hSno + ')"';
                        }
                    }
                    html += '>';
