	...
	<div id="container" class="no-print">	
		<div class="card card-body">    
        	...
        	<div class="row d-flex my-auto">    	
					...
					<?php $select_array = array("chk_department_name_hindi" => "विभाग", "chk_project_name_hindi" => "कार्य  का नाम (हिन्दी ) ", "chk_district"=>"जिला", "chk_go_no" => "शाशनादेश संख्या", "chk_financial_go_date" => "शाशनादेश की तिथि", "chk_sacntion_cost" => "कुल  लागत ( लाख में )","chk_balance_amount_refer" => "अवमुक्त की गई धनराशि ( लाख में )", "chk_project_status_1" => "परियोजना की स्थिति  1", "chk_remarks" => "अभियुक्ति" );
					?>
					...
					<table class="table table-striped table-hover table-bordered" id="general_stat_table">
						...
						//if($_SESSION['usertype']=='sadmin'){
							...
							while($row = mysqli_fetch_assoc($result)){
								...
								if(mysqli_num_rows($restotrecamount)==0){
									$totrec = (float)$row['rcv_tot_amount'];
									$totrest = (float)$row['balance_amt_project'];
									$status_1 = $row['project_status_1'];									
								}
								else{
									...
									$status_1 = $rowtotrecamount['project_status_1'];									$status_date = $rowtotrecamount['status_1_date'];
								}

								$sql = 'select * from master_projoect_status_1 where sno="'.$status_1.'"';
								//echo $row['sno'].'>>'.$sql.'<br>'; 
								...
								if(isset($_POST['chk_district'])){
								    echo '<td>'.$row['district_name_hindi'].'</td>';
								}
								if(isset($_POST['chk_go_no'])){
								...
								if(isset($_POST['chk_project_status_1'])){
								    ...
								    $statusDate = strtotime($rowtotrecamount['status_1_date']);										if ($statusDate !== false) {											echo '<td>'.$status1['status_1'].'</br>'.date("d-m-Y", $statusDate).'</td>';										} else {											echo '<td>'.$status1['status_1'].'</br>-</td>';										}
								}

								if(isset($_POST['chk_remarks'])){
								    echo '<td>'.$rowtotrecamount['remark'].'</td>';
								}
								...
								$tot_cost += (float)$row['sanction_cost'];
								$tot_recv += (float)$totrec;
							
							}
							echo '<tfoot><tr><th colspan="5"></th><th>Total:</th><th>'.round($tot_cost,2).'</th><th>'.round($tot_recv,2).'</th><th colspan="3"></th></tr></tfoot>';
						//}				
