		...
		<div class="card card-body">
			<div class="row d-flex my-auto">
				<table width="100%" class="table table-striped table-hover rounded">
					<tr>
						<th width="14%">Date Type</th>
						<th width="10%"><select name="date_type" id="date_type" class="form-control">
						...
						<th width="10%">Form</th>
						<th width="10%"><input type="date" name="date_from" id="date_from"
								value="<?php echo $_POST['date_from']; ?>" class="form-control"></th>
						<th width="10%">To</th>
						<th width="10%"><input type="date" name="date_to" id="date_to"
					...
					<tr>
						...
						<th width="15%">
							<select class="form-control" name="department" id="department"
								tabindex="<?php echo $tab++; ?>" onChange="fill_sub_department(this.value), fill_scheme(this.value)">
								<option value="">--- Select ---</option>
								<?php
						...
						<th width="10%">
							<select class="form-control" name="sub_department" id="sub_department"
								tabindex="<?php echo $tab++; ?>" onChange="fill_scheme_row($('#department').val());">
								<option value="">--- Select ---</option>
								<?php
					...
					<div class="card">
						...
						<div class="card-body" style="position:relative;">
							<table class="table table-striped table-hover table-bordered" id="general_stat_table"
								border="1" style="position:sticky;top:0;">
								<thead style="position:sticky;top:0; z-index:2;">
									<tr>
										<th rowspan="2">क्रम.</th>
										<th rowspan="2">ज़ोन का नाम</th>
									...
									<tr>
										<th>दिनांक </th>
										<th>धनराशि </th>
										<th>पर वर्तमान माह में व्यय </th>
										<th>वर्तमान वित्तीय वर्ष में कुल व्यय </th>
								...
								<tbody>
									...
									$i = 1;
									if (isset($_POST['search'])) {
										$sql = 'select uprnss_project_temp.sno as sno, division_name,sub_department_id,reporting_status, district_name_hindi, department_name_hindi, project_name, project_name_hindi, sanction_date, sanction_cost,revised_date, revised_cost, work_start_date, work_completion_date,state_share,central_share, admin_go_no, admin_go_date, financial_go_no, financial_go_date, financial_go_amount, technical_sanction_date, technical_sanction_no, last_update,status,uprnss_division.zone_id, master_freeze from uprnss_project_temp 
								...
								where (status!="5" or status="0"  or status is null or status="1")';
										...
										if (isset($_POST['search'])) {
											...
											if ($_POST['scheme'] != '') {
												$sql .= ' and scheme="' . $_POST['scheme'] . '"';
											}
											if ($_POST['sub_scheme'] != '') {
												$sql .= ' and sub_scheme="' . $_POST['sub_scheme'] . '"';
											}
										...
										<td>' . $row['admin_go_no'] . '</td>'; ?>
											...
											<td>
												...
												if (!empty($row['land_receive_date'])) {
													echo date('d-m-Y', strtotime($row['land_receive_date']));
												} else {
													echo '';
												}
												?>
										...
										<td>' . $row['sanction_cost'] . '</td>'; ?>
											...
											<td>
												...
												echo $row_invoice['revised_cost'];
												echo '</br>';
												if (!empty($row_invoice['revised_date'])) {
													echo date('d-m-Y', strtotime($row_invoice['revised_date']));
												} else {
													echo '';
												}
												?>
										...
										'; ?>
											<td>
												...
												echo $row['technical_sanction_no'];
												echo '</br>';
												if (!empty($row['technical_sanction_date'])) {
													echo date('d-m-Y', strtotime($row['technical_sanction_date']));
												} else {
													echo '';
												}
												?>
										...
										<td>' . $row_invoice['current_month_exp'] . '</td>
										<td>' . $row_invoice['current_fy_tot_exp'] . '</td>
										<td>' . $row_invoice['tot_exp_project'] . '</td>
										<td>' . $row_invoice['balance_amt_project'] . '</td>
										...
										<td>';
											if($row_invoice['total_received_amount']==0){
												...
												$row_invoice['revised_cost_diff'] = ($row_invoice['revised_cost_diff']==''?0:$row_invoice['revised_cost_diff']);
												
												$row['sanction_cost'] = ($row['sanction_cost']==''?1:$row['sanction_cost']);
												
												echo round(($row_invoice['total_received_amount']/($row['sanction_cost']+$row_invoice['revised_cost_diff']))*100,2);
											}
									...
									echo '</td>
										...
										<td>' . $row_invoice['total_physical_progress'] . '</td>'; ?>
											<td>
												...
												if($row_invoice['project_status_1']==7 || $row_invoice['project_status_1']==8){
													echo $status1['status_1'];
													echo '</br>';
													if (!empty($row_invoice['status_1_date'])) {
														echo date('d-m-Y', strtotime($row_invoice['status_1_date']));
													} else {
														echo 'दिनांक उपलब्ध नहीं ';
													}	
												}	
		...
		function fill_sub_department(val, selected=''){
			...
			$.ajax({
				...
				success: function(ajaxdata){
					...
					$("#sub_department").html(txt);
				}
			});
		}

		function fill_scheme_row(val, selected=''){
			...
			$("#scheme").html('<option value="">--Select--</option>');
			$.ajax({
				type: "POST",
				url: actionUrl,
				data: data, // serializes the form's elements.
