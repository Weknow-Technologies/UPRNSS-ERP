					...
					<table class="table table-striped table-bordered table-hover" id="content">
						...
						<tbody>
							...
							while ($row_zone = mysqli_fetch_assoc($result_zone)) {
								...
								$sql = 'SELECT s_no FROM uprnss_division WHERE zone_id="' . $row_zone['sno'] . '"';
								$result_div = execute_query($sql);
								// echo $sql;
								$sno_array = array();
								while ($row_div = mysqli_fetch_assoc($result_div)) {
									$sno_array[] = $row_div["s_no"];
								}

								$sql = 'select count(*) c, sum(abs(sanction_cost)) as amount from uprnss_project_temp where uprnss_project_temp.division_id in (' . implode(",", $sno_array) . ') and (status!="5" or status="0" or status is null or status="1")';
								$allotted = mysqli_fetch_assoc(execute_query($sql));

								$sql = 'select count(*) c, sum(abs(sanction_cost)) as amount from uprnss_project_temp where uprnss_project_temp.division_id in (' . implode(",", $sno_array) . ') and (status!="5" or status="0" or status is null or status="1") and (project_status_1="6")';
								$lack_of_money = mysqli_fetch_assoc(execute_query($sql));

								$sql = 'select count(*) c, sum(abs(sanction_cost)) as amount from uprnss_project_temp where uprnss_project_temp.division_id in (' . implode(",", $sno_array) . ') and (status!="5" or status="0" or status is null or status="1") and (project_status_1="4")';
								$controversial = mysqli_fetch_assoc(execute_query($sql));

								$sql = 'select count(*) c, sum(abs(sanction_cost)) as amount from uprnss_project_temp where uprnss_project_temp.division_id in (' . implode(",", $sno_array) . ') and (status!="5" or status="0" or status is null or status="1") and (project_status_1="10")';
								$progress = mysqli_fetch_assoc(execute_query($sql));

								$sql = 'select count(*) c, sum(abs(sanction_cost)) as amount from uprnss_project_temp where uprnss_project_temp.division_id in (' . implode(",", $sno_array) . ') and (status!="5" or status="0" or status is null or status="1") and (project_status_1="11")';
								$complete = mysqli_fetch_assoc(execute_query($sql));

								$sql = 'select count(*) c, sum(abs(sanction_cost)) as amount from uprnss_project_temp where uprnss_project_temp.division_id in (' . implode(",", $sno_array) . ') and (status!="5" or status="0" or status is null or status="1") and (project_status_1="7")';
								$inventory_sent = mysqli_fetch_assoc(execute_query($sql));

								$sql = 'select count(*) c, sum(abs(sanction_cost)) as amount from uprnss_project_temp where uprnss_project_temp.division_id in (' . implode(",", $sno_array) . ') and (status!="5" or status="0" or status is null or status="1") and (project_status_1="8")';
								$handover = mysqli_fetch_assoc(execute_query($sql));

								$sql = 'select * from uprnss_project_temp where uprnss_project_temp.division_id in (' . implode(",", $sno_array) . ') and (status!="5" or status="0" or status is null or status="1") and (project_status_1="11" or project_status_1="7" or project_status_1="8")';
								$ci_complete = mysqli_num_rows(execute_query($sql));

								$sql = 'select count(*) c, sum(abs(sanction_cost)) as amount from uprnss_project_temp where uprnss_project_temp.division_id in (' . implode(",", $sno_array) . ') and (status!="5" or status="0" or status is null or status="1") and (project_status_1="" or project_status_1 is null or project_status_1="3" or project_status_1="9" or project_status_1="5" or project_status_1="12" or project_status_1="13")';
								...
								echo '<tr>
										<td>' . $i++ . '</td>
										<td><a href="dashboard_zone.php?id=' . $row_zone['sno'] . '" target="_blank">' . $row_zone['zone_name'] . '</a></td>
										<td>' . $allotted['c'] . '</td>
...
<script>
	...
	function downloadPDF() {
		...
		html2pdf().from(element).save('zone_wise_report.pdf');
	}
</script>
