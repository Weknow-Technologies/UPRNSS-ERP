                ...
                <div class="card">
                    ...
                    <div class="card-body">
						<table class="table table-hover table-striped table-bordered">
							...
							<tbody>
							<?php
							$sql = 'SELECT uprnss_project_temp.sno as sno, project_name_hindi, master_freeze, department_name_hindi, division_name, district_name_hindi, admin_go_no  FROM `uprnss_project_temp` 
							...
							where department_id in ('.$_GET['id'].') and (status!="5" or status="0" or status is null or status="1")  ORDER BY `uprnss_division`.`division_name` ASC ';
							// echo $sql.'<br>';
							...
							while($row = mysqli_fetch_assoc($result)){
								...
								$sql = 'SELECT * FROM `invoice_civil` where project_name="'.$row['sno'].'"  and last_update>="'.date("Y-m-14").'" and last_update<="'.date("Y-m-t").'" order by last_update desc limit 1';
								// echo $sql.'<br>';
