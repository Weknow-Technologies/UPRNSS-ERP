...
if(isset($_POST['submit'])){
	if($_POST['edit_sno']==''){
		...
		$sql = 'update invoice_prarup_new_6 set 
			`total_trans`= "'.$_POST['prarup_new_6_id'].'",
			...
			if(mysqli_error($db)){ 
				...
				$sql = 'delete from trans_prarup_new_6 where invoice_id="'.$_POST['edit_sno'].'"';
				// echo $sql;
				execute_query($sql);
				for($i=1; $i<=$_POST['prarup_new_6_id']; $i++){
					if($_POST['month_'.$i]!=""){
						$sql = 'insert into trans_prarup_new_6 (`invoice_id`, `month`,`avsheshdate`, `totelpayment`, `tenderfee`, `tenderfeemonth`, `utrdate`,  `utrnumber`, `maintenderfee`, `avsheshtenderfee`, `otherremark`, created_by, creation_time) values ("'.$_POST['edit_sno'].'", "'.$_POST['month_'.$i].'","'.$_POST['avsheshdate_'.$i].'", "'.$_POST['totelpayment_'.$i].'", "'.$_POST['tenderfee_'.$i].'", "'.$_POST['tenderfeemonth_'.$i].'", "'.$_POST['utrdate_'.$i].'", "'.$_POST['utrnumber_'.$i].'", "'.$_POST['maintenderfee_'.$i].'", "'.$_POST['avsheshtenderfee_'.$i].'", "'.$_POST['otherremark_'.$i].'", "'.$_SESSION['username'].'", "'.date("Y-m-d H:i:s").'");';
						execute_query($sql);
						if(mysqli_error($db)){ 
							$msg .= '<p class="text text-danger">Error # 1 : '.mysqli_error($db).'>> '.$sql.'</p>';
						}
	...
	$_POST['tenderfee_1']="";
	$_POST['tenderfeemonth_1']="";
	$_POST['utrdate_1']="";
	$_POST['utrnumber_1']="";
	$_POST['maintenderfee_1']="";
	$_POST['avsheshtenderfee_1']="";
	$_POST['otherremark_1']="";
	
	$_POST['prarup_new_6_id']="1";
	$_POST['edit_sno']="";
}

	if(isset($_GET['edit_sno'])){
		$sql = 'select * from trans_prarup_new_6 where invoice_id="'.$_GET['edit_sno'].'"';
		// echo $sql.'<br>';
		$result_rcpt = execute_query($sql);
		if(mysqli_num_rows($result_rcpt)!=0){
			$_POST['prarup_new_6_id'] = mysqli_num_rows($result_rcpt);
			$i=1;
			while($row_rcpt = mysqli_fetch_assoc($result_rcpt)){
				// echo '<h1>Test '.$i.'</h1>'.$_POST['prarup_new_6_id'].'<br/>';
				
				$_POST['month_'.$i] =$row_rcpt['month'];
				$_POST['avsheshdate_'.$i] =$row_rcpt['avsheshdate'];
				$_POST['totelpayment_'.$i] =$row_rcpt['totelpayment'];
				$_POST['tenderfee_'.$i] =$row_rcpt['tenderfee'];
				$_POST['tenderfeemonth_'.$i] =$row_rcpt['tenderfeemonth'];
				$_POST['utrdate_'.$i] =$row_rcpt['utrdate'];
				$_POST['utrnumber_'.$i] =$row_rcpt['utrnumber'];
				$_POST['maintenderfee_'.$i] =$row_rcpt['maintenderfee'];
				$_POST['avsheshtenderfee_'.$i] =$row_rcpt['avsheshtenderfee'];
				$_POST['otherremark_'.$i] =$row_rcpt['otherremark'];
            ...
            <div class="col-md-12">
				<div class="card">
					<div class="card-header">
						<h4 class="card-title text-center bg-danger text-white p-2">मुख्यालय को टेंडर फीस का विवरण </h4></br>
					</div>
						<?php echo $msg; ?>
					<div class="card-body">
						<div class="row">
						...
						<div class="row border rounded m-2 p-2 border-secondary">
							...
							<div class="col-md-3">
								...
								<input type="number" class="form-control" name="avsheshdate_<?php echo $i; ?>" id="avsheshdate_<?php echo $i; ?>" value="<?php echo $_POST['avsheshdate_'.$i]; ?>">
							</div>
							<div class="col-md-3">
								<label>टेण्डर के सापेक्ष माह में प्राप्त टेण्डर फीस</label>
								<input type="number" class="form-control" name="totelpayment_<?php echo $i; ?>" id="totelpayment_<?php echo $i; ?>" value="<?php echo $_POST['totelpayment_'.$i]; ?>">
							</div>
							<div class="col-md-3">
								<label>टेण्डर के सापेक्ष क्रमिक प्राप्त टेण्डर फीस</label>
								<input type="number" class="form-control" onInput="addCalc()" name="tenderfee_<?php echo $i; ?>" id="tenderfee_<?php echo $i; ?>" value="<?php echo $_POST['tenderfee_'.$i]; ?>">
							</div>
							<div class="col-md-3">
								<label>माह में मुख्यालय को प्रेषित टेण्डर फीस</label>
								<input type="number" class="form-control" name="tenderfeemonth_<?php echo $i; ?>" id="tenderfeemonth_<?php echo $i; ?>" value="<?php echo $_POST['tenderfeemonth_'.$i]; ?>">
							</div>
							<div class="col-md-3">
								<label>यू०टी०आर०नम्बर</label>
								<input type="text" class="form-control" name="utrnumber_<?php echo $i; ?>" id="utrnumber_<?php echo $i; ?>" value="<?php echo $_POST['utrnumber_'.$i]; ?>">
							</div>
							<div class="col-md-3">
								<label>यू०टी०आर०  दिनांक</label>
								<input type="date" class="form-control" name="utrdate_<?php echo $i; ?>" id="utrdate_<?php echo $i; ?>" value="<?php echo $_POST['utrdate_'.$i]; ?>">
							</div>
							<div class="col-md-3">
								<label>मुख्यालय को क्रमिक प्रेषित टेण्डर फीस</label>
								<input type="number" class="form-control" onInput="addCalc()" name="maintenderfee_<?php echo $i; ?>" id="maintenderfee_<?php echo $i; ?>" value="<?php echo $_POST['maintenderfee_'.$i]; ?>">
							</div>
							<div class="col-md-3">
								<label>अवशेष टेण्डर फीस</label>
								<input readonly type="number" class="form-control" name="avsheshtenderfee_<?php echo $i; ?>" id="avsheshtenderfee_<?php echo $i; ?>" value="<?php echo $_POST['avsheshtenderfee_'.$i]; ?>">
							</div>
							<div class="col-md-3">
								<label>अन्य विवरण</label>
								<input type="text" class="form-control" name="otherremark_<?php echo $i; ?>" id="otherremark_<?php echo $i; ?>" value="<?php echo $_POST['otherremark_'.$i]; ?>">
							</div>
						...
						?>
						<input type="hidden" name="prarup_new_6_id" id="prarup_new_6_id" value="<?php echo $_POST['prarup_new_6_id']; ?>">
							...
							<div class="form-group">
								...
								<?php 
									if (isset($_GET['edit_sno']) && $_GET['edit_sno'] != "") {
										...
										<a href="prarup_new_6.php" class="btn btn-info">Back</a>';
									}else{
...
console.log(id);
			for (let i = 1; i <= id; i++) {
				let tenderfee = parseFloat(document.getElementById("tenderfee_" + i).value);
				if (!tenderfee) {
					tenderfee = 0;
				}

				let maintenderfee = parseFloat(document.getElementById("maintenderfee_" + i).value);
				if (!maintenderfee) {
					maintenderfee = 0;
				}
				
				var ans=tenderfee-maintenderfee;
				document.getElementById("avsheshtenderfee_"+i).value=ans.toFixed(2);
				// console.log(ans);
			}
		}
					...
					<div class="card-body">
						<table class="table table-striped table-bordered table-hover">
							...
							if($_SESSION['usertype']=='1'){
								...
								$sql = 'SELECT sno,division_id, entry_date, created_by  FROM `invoice_prarup_new_6` ORDER BY division_id';
							}
							...
							while($row_prarup_new_6 = mysqli_fetch_assoc($result_prarup_new_6)){
								...
								<td><a href="'.$_SERVER["PHP_SELF"].'?delid='.$row_prarup_new_6['sno'].'"  onClick="return confirm(\'Are you sure you? you want to Delete\');"><i class="far fa-trash-alt"></i></a></td>
								<td><a href="'.$_SERVER["PHP_SELF"].'?edit_sno='.$row_prarup_new_6['sno'].'" ><i class="far fa-edit"></i></a></td>
								</tr>';
							}
							?>
							</tbody>
