...
if(isset($_GET['id'])){
	$sql = 'SELECT * FROM `billit_invoice_cash_voucher` AS `bicv` JOIN `billit_cash_voucher_journal` AS `bcvj` ON `bicv`.`sno` = `bcvj`.`journal_id` WHERE `bicv`.`sno` ="'.$_GET['id'].'"';
		  		...
		  		<div class="col-12">
					 ...
					 <table class=" table table-striped table-hover table-bordered">
						...
						<tr>
							...
							<td>Treat as Final : <input type="checkbox" name="treat_as_final" <?php if(isset($_GET['id'])){if(isset($old_data['status']) && $old_data['status']==1){echo "checked='checked'";}}else{ echo 'checked="checked"';}?>></td>
							<td class="text-right">
								<a href="cash_voucher.php?view=view"><button type="button" class="btn btn-warning">View Cash Vouchers</button></a>
							</td>
						</tr>
						<tr>
							<td>Date</td>
							...
							<td>Voucher No.</td>
							<td><input id="challan_no" name="challan_no" class="field text" size="12" maxlength="18" tabindex="<?php echo $tab++; ?>" type="text" value="<?php if(isset($_GET['id'])){echo $old_data['voucher_no'];}?>"></td>
						</tr>
					</table>
				</div>
		  	</div>
		  	<div class="card pb-5 px-2">
						...
						<div class="row" id="row_1">
								...
								<select name="vendor_1" id="vendor_1" class="form-control" onFocus="set_current(1)">
									...
									if ($result && mysqli_num_rows($result) > 0) {
										while ($row = mysqli_fetch_assoc($result)) {
                                            ...
                                            $selected = "";
                                            if(isset($_GET['id']) && $old_data['vendor'] == $name){
                                                $selected = "selected";
                                            }
							...
							<div class="col-md-3 mb-2">
								...
								<input type="text" name="description" id="description_1" class="form-control"
									placeholder="Enter Description" onBlur="insert_row(1);" onFocus="set_current(1)"
									value="<?php if(isset($_GET['id'])){ echo $old_data['remarks']; } ?>">
							</div>
							<div class="col-md-3 mb-2">
								<label>&nbsp;</label>
								<button type="button" class="btn btn-danger btn-sm" onclick="remove_row(1)" id="remove_btn_1" style="display:none;">Remove</button>
							</div>
						</div>
						<div class="row" id="row_ledger_1">
							<div class="col-1">
								<label for="">By</label>
								<select name="voucher_type_1" id="voucher_type_1" class="form-control" onFocus="set_current(1)" onChange="update_voucher(1);">
							...
							<div class="col-4">
								...
								<input type="text" name="account_1" id="account_1" class="form-control" onFocus="set_current(1)">
							...
							<div class="col-2">
								...
								<input type="text" name="debit_1" id="debit_1" class="form-control" placeholder="Amount" onFocus="set_current(1)">
							</div>
							<div class="col-2">
								<label for="">Credit Amount</label>
								<input type="text" name="credit_1" id="credit_1" class="form-control" placeholder="Amount" disabled onFocus="set_current(1)">
							</div>
							<div class="col-3">
								<label for="">Description</label>
								<input type="text" name="description_1" id="description_1" class="form-control" placeholder="Description" onFocus="set_current(1)">
							</div>
						</div>
					    </div>
				<div class="row mt-3">
					<div class="col-12">
						...
						<button type="button" class="btn btn-info" data-toggle="modal" data-target="#createModal">New Ledger <i class="far fa-plus-square"></i></button>
					...
					<div class="col-12 ">
						<div class="row justify-content-end no-gutters my-3">
							...
							<div class="col-2 px-0">
								<div class="alert alert-warning text-dark mb-0">&nbsp;<span id="total_damt"></span></div>
							</div>
							<div class="col-2 px-0">
								<div class="alert alert-success mb-0" style="margin-left: -15px; margin-right: 10px; padding-left: 10px; padding-right: 10px;">&nbsp;<span id="total_camt"></span></div>
							</div>
						</div>
					</div>
				</div>
			    <div class="row">
                    <div class="col-12 text-left">
                        <input id="saveForm" name="saveForm" class="btn btn-primary" type="submit" value="Submit" tabindex="<?php echo $tab;?>" >
                    </div>
                </div>
                            ...
                            <td class="no-print actions-col" style="white-space:nowrap">
                                <div class="dropdown">
                                  ...
                                  <div class="dropdown-menu dropdown-menu-right">
                                    ...
                                    <a class="dropdown-item" target="_blank" href="cash_voucher_print.php?id=' . $row['sno'] . '">👁️ View Details</a>
