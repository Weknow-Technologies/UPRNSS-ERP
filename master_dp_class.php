...
if(isset($_POST['submit'])){
	if($_POST['edit_sno']==''){
		$sql = 'insert into dp_class (class_name) values ("'.$_POST['class_name'].'")';
		execute_query($sql);
		if(mysqli_error($db)){ 
...
if(isset($_GET['edit_sno'])){
	$sql = 'select * from dp_class where sno="'.$_GET['edit_sno'].'"';
	$data = mysqli_fetch_assoc(execute_query($sql));
	$_POST['class_name'] = $data['class_name'];
	$_POST['edit_sno'] = $data['sno'];
	
}	

if(isset($_GET['del'])){
	$sql = 'delete from dp_class where sno="'.$_GET['del'].'"';
	execute_query($sql);
						...
						<div class="row" id="">
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="class_name" id="class_name" class="form-control" placeholder="" value="<?php echo $_POST['class_name']; ?>" tabindex="<?php echo $tab++; ?>">
                                </div>
                            </div>
                        </div>
						<div id="test"></div>
