...
if(isset($_POST['supplier1_sno'])){
	...
	if($msg==''){
		...
		$sql = 'INSERT INTO `billit_invoice_receipt` (`amount`, `mop`, `chq_no`, `timestamp`, `remarks`, `account`, `created_by`, `creation_time`, `branch`) VALUES ("'.$_POST['total_value_hidden'].'", "'.$_POST['type1'].'", "'.$_POST['chq_number'].'", "'.$_POST['sale_date'].'",  "'.$_POST['remarks1'].'",  "'.$_POST['account'].'",  "'.$_SESSION['username'].'",  "'.date("Y-m-d H:i:s").'",  "'.$_SESSION['usersno'].'");';
		execute_query($sql);
		if(mysqli_error($db)){
		...
		for($i=1;$i<$_POST['id'];$i++){
			$sql = 'INSERT INTO `billit_stock_receipt` (`invoice_no`, `customer_id`, `description`, `amount`, `mop`, `chq_no`, `timestamp`, `branch`, `account`, `status`, `created_by`, `creation_time`) VALUES ("'.$id.'", "'.$_POST['supplier'.$i.'_sno'].'", "'.$_POST['description'.$i].'", "'.$_POST['amount'.$i].'", "'.$_POST['type1'].'", "'.$_POST['chq_number'].'", "'.$_POST['sale_date'].'",  "'.$_SESSION['usersno'].'",  "'.$_POST['account'].'",  "0", "'.$_SESSION['username'].'",  "'.date("Y-m-d H:i:s").'");';
			execute_query($sql);
			if(mysqli_error($db)){
				$msg .= '<div class="alert alert-danger">Error # 1.0987 : '.mysqli_error($db).' >> '.$sql.'</div>';
			}
...
$(function() {
	var options = {
		source: "scripts/billit_ajax.php?id=cust_name",
		minLength: 1,
	...
	$(document.body).on("keydown.autocomplete", "#supplier", function(){
							...
							<div class="row px-3">
								...
								<div class="col-md-3">
									<div class="form-group mx-4">
										...
										<input type="hidden" id="supplier1_sno" name="supplier1_sno" value="<?php if(isset($_POST['sale_date'])){echo $_POST['supplier1_sno'];}else{if(isset($_GET['id'])){echo $payment['cust_id'];}} ?>">
									</div>
								</div>
								<div class="col-md-3">
									<div class="form-group mx-4">
