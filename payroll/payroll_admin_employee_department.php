...
if(isset($_POST['submit'])){
	if($_POST['edit_sno']!=''){
		$sql='UPDATE `employee_department` SET `department_name`="'.$_POST['department_name'].'", `edited_by`="'.$_SESSION['username'].'" , `edition_time`="'.date('Y-m-d h:i:s').'"  WHERE sno="'.$_POST['edit_sno'].'"';
		//echo $sql;
		...
		$sql='INSERT INTO `employee_department`(`department_name` , `created_by` , `creation_time`) VALUES ("'.$_POST['department_name'].'" , "'.$_SESSION['username'].'" , "'.date('Y-m-d h:i:s').'")';
		if ($db->query($sql) === TRUE) {
...
if(isset($_GET['del'])){
	$sql ="delete from employee_department where sno='".$_GET['del']."'";
...
if(isset($_GET['id'])){
	$sql ="select * from employee_department where sno='".$_GET['id']."'";
	$query = execute_query($sql);
	$row_edit = mysqli_fetch_array($query);
}
page_header_start('Employee Department');
page_header_end();
...
<div class="container">
	<div class="row">
		...
		<form method="post" action="<?php echo $_SERVER['PHP_SELF'];?>" enctype="multipart/form-data">
			<div class="panel">
				<div class="panel-heading">Employee Department </div>
						...
						<div class="col-sm-6">
							<table class="table table-hover table-responsive table-condensed">
								<tr>
									...
									<td>
										<input type="text" name="department_name" placeholder="Enter Department name" value="<?php 
										if(isset($_GET['id'])){echo $row_edit['department_name']; }?>" class="form-control" tabindex="1"  required>
									</td>
								</tr>
							</table>
						</div>
