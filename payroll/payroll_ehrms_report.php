...
page_header_start('EHRMS Report');
				...
				<form  method="post" action="<?php echo $_SERVER['PHP_SELF'];?>"  name="confirm" enctype="multipart/form-data">
					...
					<div class="panel-body">
						...
						<div style="text-align: right; width: 100%;">
							...
							<a href="payroll_ehrms_report_export.php" target="_blank" class="no-print btn btn-info"
		...
		<div class="row">
			...
			<thead>
				<tr>
					...
					<th rowspan="2" style='border:2px solid black;'>Emp Code</th>
					<th rowspan="2" style='border:2px solid black;'>Emp Name</th>
					<th rowspan="2" style='border:2px solid black;'>Emp Location</th>
					<th rowspan="2" style='border:2px solid black;'>EHRMSID</th>
					<th rowspan="2" style='border:2px solid black;'>Month</th>
					<th rowspan="2" style='border:2px solid black;'>Year</th>
					<th rowspan="3" style='border:2px solid black;'>Gross Salary Emp</th>
					<th rowspan="3" style='border:2px solid black;'>Net Salary Emp</th>
			...
			<tbody>
				<?php
				$sql ='SELECT `payslip_report`.`sno` as payslip_id , `employee`.`sno` as emp_id , `employee`.`employee_name` ,`employee`.`company_id` ,`employee`.`employee_code` ,`employee`.`bank_name` ,`employee`.`acount_number` ,`employee`.`ifsc_code` ,`employee`.`ehrms_id` , `employee`.`employee_designation` , `payslip_report`.`financial_year` , `payslip_report`.`for_month`, `payslip_report`.`total_income` , `payslip_report`.`advance_amount` , `payslip_report`.`total_deduction` , `payslip_report`.`total` FROM `payslip_report` join `employee` on `employee`.`sno` = `payslip_report`.`emp_id`  WHERE 1=1 ';
				...
				while($row=mysqli_fetch_array($result)){ 
					if($row['total'] > 0){
						...
						<td style='border:2px solid black;'>".$row["ehrms_id"]."</td>
						<td style='border:2px solid black;'>".$row["for_month"]."</td>
						<td style='border:2px solid black;'>".$row['financial_year']."</td>
						<td style='border:2px solid black;'>".$row["total_income"]."</td>
						
						<td style='border:2px solid black;'>".$row["total"]."</td>";
						$total_grand += $row['total'];
						$total_net += $row['total_income'];
				...
				<tr>
					...
					<th colspan="7" style="text-align: center;border:2px solid black;">Total:</th>
					<th style='border:2px solid black;'><?php echo round($total_net); ?></th>
