...
$sheet->setCellValue('J3', 'Deduction');
...
$row_deduction = mysqli_fetch_assoc($result_deduction);

$sheet->setCellValue('J4', $row_deduction['head_name']);
...
while ($row = mysqli_fetch_array($result)) {
    ...
    if ($deduction_re > 0) {
		...
		$sheet->setCellValue('J'.$row_count, $row_amount_deduction['amount']);
        $total_grand += (float)$row_amount_deduction['amount'];
		$row_count++;
	}
