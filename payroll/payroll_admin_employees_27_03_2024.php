...
if(isset($_POST['submit'])) {
	if($_POST['edit_sno'] != ''){
		...
		if($_POST['working_status']!=0){
			$working_condition = ' working_status=1, exit_date="'.$_POST['exit_date'].'", ';
		}
		else{
			$working_condition='';
		}
		...
		// $sql = "SELECT * FROM `Employee` WHERE dp_name_id = '" . $_POST['dp_name_id'] . "'";
		// $result=execute_query($sql);
		// $rowcount=mysqli_num_rows($result);
		// if($rowcount>0){
			// $msg='<div class="alert alert-danger">Alredy Save !</div>';
		// }
		// else{
		$sql = 'INSERT INTO `employee` (dp_name_id, employee_category_id, employee_code, `employee_name`, `pay_level_id`, `pan`, `esic_no`, `employee_designation`, `aadhar`, `acount_number`, `bank_name`,`branch`, `ifsc_code` , `company_id`,`agmt_started_date` , `agmt_ending_date` , `agmt_order_no` , `created_by` , `creation_time`, `working_status`) 
			VALUES ("'.$_POST['dp_name_id'].'", "'.$_POST['employee_category_id'].'", "'.$_POST['employee_code'].'", "'.$_POST['employee_name'].'", "'.$_POST['pay_level_id'].'","'.$_POST['pan'].'",  "'.$_POST['esic_no'].'",  "'.$_POST['employee_des'].'", "'.$_POST['aadhar'].'", "'.$_POST['acount_number'].'", "'.$_POST['bank_name'].'", "'.$_POST['branch'].'", "'.$_POST['ifsc_code'].'" , "'.$_POST['company_id'].'" , "'.$_POST['agmt_started_date'].'" , "'.$_POST['agmt_ending_date'].'" ,"'.$_POST['agmt_order_no'].'" , "'.$_SESSION['username'].'" , "'.date('Y-m-d h:i:s').'" , "0")';
			execute_query($sql);
			if(mysqli_error($db)){
				...
				<div class="panel">
					...
					<div class="panel-body">
						<div class="row">
							<div class="col-sm-4">
								Working Status:
								<select name="working_status" id="working_status" class="form-control" placeholder="" tabindex="<?php echo $tab++;?>" onChange="working_condition(this.value);">
									<option value="0">Working</option>
									<option value="1">Left</option>
								</select>
							</div>
							<div class="col-sm-4" id="exit_date_row" style="display: none;">
								Leaving Date:
								<script type="text/javascript" language="javascript">
									document.writeln(DateInput('exit_date', 'admin_employee_from', true, 'YYYY-MM-DD', '<?php echo date("Y-m-d"); ?>', <?php echo $tab++; $tab=$tab+3; ?>));
								</script> 
							</div>
							<div class="col-sm-4">
						...
						<div class="col-sm-4">
							Division Name: 
							<select name="company_id" id="company_id" class="form-control" tabindex="<?php echo $tab++; ?>" >
						...
						<select name="employee_category_id" id="employee_category_id" class="form-control" tabindex="<?php echo $tab++; ?>" onchange="dynamicHead(this.value)" >
								<option value="">-SELECT-</option>
								<?php 
						...
						<div class="col-sm-4">
							Pay Level:  
							<select name="pay_level_id" class="form-control" tabindex="<?php echo $tab++; ?>" >
								<option value="">-SELECT-</option>
								<?php 
