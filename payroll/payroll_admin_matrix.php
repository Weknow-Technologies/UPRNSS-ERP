...
if(isset($_POST['submit'])){
	if($_POST['edit_sno']!=''){
		$sql='UPDATE `pay_matrix` SET `matrix_name`="'.$_POST['matrix_name'].'", `edited_by`="'.$_SESSION['username'].'" , `edition_time`="'.date('Y-m-d h:i:s').'"  WHERE sno="'.$_POST['edit_sno'].'"';
		//echo $sql;
		...
		$sql='INSERT INTO `pay_matrix`(`matrix_name` , `created_by` , `creation_time`) VALUES ("'.$_POST['matrix_name'].'" , "'.$_SESSION['username'].'" , "'.date('Y-m-d h:i:s').'")';
		if ($db->query($sql) === TRUE) {
...
if(isset($_GET['del'])){
	$sql ="delete from pay_matrix where sno='".$_GET['del']."'";
...
if(isset($_GET['id'])){
	$sql ="select * from pay_matrix where sno='".$_GET['id']."'";
	$query = execute_query($sql);
	$row_edit = mysqli_fetch_array($query);
}
page_header_start('Pay Matrix');
page_header_end();
...
<div class="container">
	<div class="row">
		...
		<form method="post" action="<?php echo $_SERVER['PHP_SELF'];?>" enctype="multipart/form-data">
			<div class="panel">
				<div class="panel-heading">Pay Matrix</div>
						...
						<div class="col-sm-6">
							<table class="table table-hover table-responsive table-condensed">
								<tr>
									<td >Matrix Name *</td>
									<td>
										<input type="text" name="matrix_name" placeholder="Enter Matrix Name" value="<?php 
										if(isset($_GET['id'])){echo $row_edit['matrix_name']; }?>" class="form-control" tabindex="1"  required>
									</td>
								</tr>
							</table>
						</div>
