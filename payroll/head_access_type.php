...
if(isset($_POST['submit'])){
	//print_r($_POST);
	if($_POST['edit_sno']==''){
		$sql = 'insert into payroll_emp_category (category_name) values ("'.$_POST['cat_type'].'")';
	}
	else{
		$sql = 'update payroll_emp_category set 
		category_name = "'.$_POST['cat_type'].'"
	...
	if(mysqli_error($db)){ 
		...
		execute_query($sql);
		if(isset($_POST['head_access'])){
			foreach($_POST['head_access'] as $k=>$v){
				...
				execute_query($sql);
				if(mysqli_error($db)){ 
					$msg .= '<p class="alert alert-danger">Error # 2 : '.mysqli_error($db).'>> '.$sql.'</p>';
				}
			}
		}
...
if(isset($_GET['eid'])){
	...
	$_POST['cat_type'] = $data['category_name'];
...
if(isset($_GET['delid'])){
	...
	$sql = 'delete from payroll_head_access where cat_id="'.$_GET['delid'].'"';
	execute_query($sql);
	$msg .= '<p class="alert alert-danger">Data Deleted.</p>';
}
...
?>
   ...
   <div class="panel">
		<div class="panel-heading">Head Access</div>
				...
				<div class="col-md-2"></div>
					<div class="col-md-3">
						<div class="form-group">
							...
							<input type="text" name="cat_type" id="cat_type" class="form-control" tabindex="<?php echo $tab++; ?>" value="<?php echo $_POST['cat_type']; ?>">
						</div>
					</div>
					<div class="col-md-5">
						<div class="form-group">
							<label >Head Access</label>
							<select name="head_access[]" id="head_access" class="form-control" tabindex="<?php echo $tab++; ?>" multiple>
								<?php
								$sql = 'select * from head_type';
								$result = execute_query($sql);
								while($row = mysqli_fetch_assoc($result)){
									echo '<option value="'.$row['sno'].'" ';
									if(in_array($row['sno'], $_POST['head_access'])){
		...
		<div class="row">
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
					<table class="table table-hover table-responsive table-bordered table-striped" id="general_stat_table" style="width:70%;margin:auto;">
						<thead>
						...
						while($row = mysqli_fetch_assoc($result)){
							...
							$access = array();
							$sql = 'select head_name from payroll_head_access left join head_type on head_type.sno = payroll_head_access.head_id where cat_id="'.$row['sno'].'"';
							//echo $sql;
