...
if(isset( $_POST['submit'])){
	...
	elseif($_POST['edit_sno'] != '' and $_POST['edit_sno'] != '1'){
		...
		if ($db->query($sql) === TRUE) {
			...
			if(isset($_POST['value_type']) && $_POST['value_type']=="other" ){
				
				$sql = 'delete from head_type_formula where head_id="'.$_POST['edit_sno'].'"';
				// echo $sql;
				execute_query($sql);
				for($i=1;$i<=$_POST['id'];$i++){
					if($_POST['var_a_'.$i]!=$_POST['var_b_'.$i]){
						$sql = 'insert into head_type_formula (`head_id`, `var_a`, `operator`, `var_b`, `step`, `round_off`) values ("'.$_POST['edit_sno'].'", "'.$_POST['var_a_1'].'", "'.$_POST['calc_1'].'", "'.$_POST['var_b_1'].'", "'.$i.'","'.$_POST['round_off'].'")';
						execute_query($sql);
						if(mysqli_error($db)){
		...
		if(!mysqli_error($db)) {
			...
			if($_POST['value_type']=='other'){
				for($i=1;$i<=$_POST['id'];$i++){
					if($_POST['var_a_'.$i]!=$_POST['var_b_'.$i]){
						$sql = 'insert into head_type_formula (head_id, var_a, operator, var_b, step,round_off) values ("'.$head_id.'", "'.$_POST['var_a_1'].'", "'.$_POST['calc_1'].'", "'.$_POST['var_b_1'].'", "'.$i.'","'.$_POST['round_off'].'")';
						execute_query($sql);
						if(mysqli_error($db)){
							$msg .= '<div class="alert alert-danger">Error # AH_003. >> '.$sql.' >> '.mysqli_error($db).'</div>';
						}
						...
						<td>
							<div id="show1">
								<select name="value_type" id="title1" class="form-control" tabindex="<?php echo $tab++; ?>">
									...
									<option value="other" <?php if(isset($_GET['id'])){if($row_edit['value_type']=="other"){echo "selected='selected'";}}?>>other</option>
								</select>
							</div>
						</td>
					</tr>
					...
					<tr id="hide3" style="display: none;">
						<td colspan="2">
							<table width="100%">
								<tr>
									<td>Formula</td>
									<td><input type="checkbox" value="1" name="round_off" id="round_off" class="form-control"></td>
									<td><select name="var_a_1" class="form-control" id="var_a_1" tabindex="<?php echo $tab++; ?>">
										<?php 
										$sql = 'SELECT * FROM `head_type`';	
										$query = execute_query($sql);
									...
									<td><input type="text" value="<?php if(isset($_GET['id'])){echo $_POST['var_b_1'];}?>" name="var_b_1" id="var_b_1" class="form-control"></td>
