						...
						<tbody>
							...
							*/	
								...
								while($row = mysqli_fetch_assoc($result)){
									...
									$sql2 = 'SELECT uprnss_division.division_name,users.user_name,users.type,user_type.user_type FROM `users` LEFT JOIN user_division on users.sno=user_division.user_id LEFT JOIN uprnss_division on user_division.division_id=uprnss_division.s_no LEFT JOIN user_type on users.type=user_type.sno where users.sno="'.$row['user_id'].'"';
									...
									<td>'.$row['datetime'].'</td>
									<td><a href="https://www.google.com/maps?q='.$row['selected_lat'].','.$row['selected_long'].'" target="_blank">'.$row['selected_lat'].' '.$row['selected_long'].'</a></td>
