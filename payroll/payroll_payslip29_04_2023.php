...
$msg12='';
	...
	if(isset($_POST['confirm_submit'])){
		...
		echo	$sql_check = 'SELECT * FROM `payslip_report` WHERE `emp_id`="'.$row_employee_attendance['emp_id'].'" AND `financial_year`="'.$_POST['financial_session'].'" AND `for_month`="'.$_POST['for_month'].'"';
			...
			if($num_check == 0){
				...
				foreach($data_array as $k=>$v){
					$sql = 'insert into payslip_details (payslip_id, head_id, amount, base_amount, head_name, head_type, value_type, percent_of, recurrence_period) values("'.$payslip_sno.'", "'.$v['head_id'].'", "'.$_POST['head_value_'.$v['head_id'].'_'.$row_employee_attendance['emp_id'].''].'", "'.$_POST['head_value_orig_'.$v['head_id'].'_'.$row_employee_attendance['emp_id'].''].'", "'.$v['head_name'].'", "'.$v['head_type'].'", "'.$v['value_type'].'", "'.$v['percent_of'].'", "'.$v['recurrence_period'].'")';
					execute_query($sql);
					//echo $sql.'<br>';
					if(mysqli_error($db)){
										...
										elseif($v['value_type']=='percent'){
											...
											if($row_employee['pay_commission']=="1"){
												if($v['head_id']=="9" || $v['head_id']=="16" || $v['head_id']=="36"){
													...
													$v['head_id']['original_value'] = $val;
													
													echo '
													<input type="hidden" name="head_value_orig_'.$v['head_id'].'_'.$row_employee['sno'].'" value="'.$v['head_id']['original_value'].'">
