...
$msg12='';
    ...
    if (isset($_POST['employee_category_id']) && $_POST['employee_category_id'] != '') {
    ...
    $saved_employees = [];
    $already_saved_employees = [];

    while ($row_employee_attendance = mysqli_fetch_array($result_employee_attendance)) {
        $sql_check = 'SELECT * FROM `payslip_report` WHERE `company_id`="' . $_POST['company_id'] . '" AND `emp_id`="' . $row_employee_attendance['emp_id'] . '" AND `financial_year`="' . $_POST['financial_session'] . '" AND `for_month`="' . $_POST['for_month'] . '"';
        ...
        if ($num_check == 0) {
            ...
            $sql_insert = 'INSERT INTO `payslip_report`(`emp_id`, `financial_year`, `main_basic`, `total_income`, `total_deduction`, `total`, `for_month`, `created_by`, `creation_time`, salary_type, working_days, presented_days, granted_leave, company_id, salary_month) VALUES ("' . $row_employee_attendance['emp_id'] . '" , "' . $_POST['financial_session'] . '" , "' . $_POST['main_basic_value_' . $row_employee_attendance['emp_id']] . '" , "' . $_POST['total_income_' . $row_employee_attendance['emp_id']] . '" ,  "' . $_POST['tot_deduction_' . $row_employee_attendance['emp_id']] . '" ,  "' . $_POST['net_pay_' . $row_employee_attendance['emp_id']] . '" , "' . $_POST['for_month'] . '" , "' . $_SESSION['username'] . '" , "' . date('Y-m-d h:i:s') . '", "main","' . $_POST['working_days_' . $row_employee_attendance['emp_id']] . '", "' . $_POST['presented_days_' . $row_employee_attendance['emp_id']] . '", "' . $_POST['granted_leave_' . $row_employee_attendance['emp_id']] . '","' . $_POST['company_id'] . '", "' . $date_to_store . '" )';
            $res = execute_query($sql_insert);

            if (!mysqli_error($db)) {
                $payslip_sno = mysqli_insert_id($db);
                $saved_employees[] = $row_employee_attendance['emp_id'];
            } else {
            ...
            foreach ($data_array as $k => $v) {
                $sql = 'insert into payslip_details (payslip_id, head_id, amount, base_amount, base_value, head_name, head_type, value_type, percent_of, recurrence_period) values(
                    "' . $payslip_sno . '", 
                    "' . $v['head_id'] . '", 
                    "' . $_POST['head_value_' . $v['head_id'] . '_' . $row_employee_attendance['emp_id']] . '", 
                    "' . $_POST['head_value_orig_' . $v['head_id'] . '_' . $row_employee_attendance['emp_id']] . '",
                    "' . $v['head_value'] . '", 
                    "' . $v['head_name'] . '", 
                    "' . $v['head_type'] . '", 
                    "' . $v['value_type'] . '", 
            ...
            $already_saved_employees[] = $row_employee_attendance['emp_id'];
        }
    }

    // Generate messages
    if (!empty($already_saved_employees)) {
        $msg .= '<div class="alert alert-info">Already Generated for Employees: ' . implode(", ", $already_saved_employees) . '</div>';
    }

    if (!empty($saved_employees)) {
        $msg .= '<div class="alert alert-success">Successfully Generated for Employees: ' . implode(", ", $saved_employees) . '</div>';
    }
}
				...
				<table class="table table-hover table-responsive table-bordered table-striped text-center" id="payroll" style="">
					...
					<tbody>
						...
						$grand_main_basic_value = 0;
							...
							//echo $sql_employee_attendance;
						while ($row_employee_attendance = mysqli_fetch_array($result_employee_attendance)) {
							if($row_employee_attendance['working_days']!=$row_employee_attendance['granted_leave']){
							...
							<tr>
								...
								<?php 
								echo'
								<input type="hidden" name="working_days_'.$row_employee['sno'].'" value="'. $row_employee_attendance['working_days'].'" >
								<input type="hidden" name="granted_leave_'.$row_employee['sno'].'" value="'. $row_employee_attendance['granted_leave'].'" >
								<input type="hidden" name="presented_days_'.$row_employee['sno'].'" value="'. $row_employee_attendance['presented_days'].'" >';
								?>
								<?php 
									$sql_salary = "SELECT *
									...
									while($row_heads = mysqli_fetch_assoc($result_salary)){
										...
										if ($_POST['employee_category_id'] == "4") {
											if ($row_heads['head_id'] == 1 ||$row_heads['head_id'] == 3 ) {
											$basic_value = $row_heads['head_value']/$row_employee_attendance['working_days']*$row_employee_attendance['presented_days'];
											$basic_value_per= $basic_value*$row_employee['salary_per']/100;
												$row_heads['head_value']=$basic_value_per;
											}
											if ($row_heads['head_id'] == 6 ||$row_heads['head_id'] == 7 ) {
											$basic_value = $row_heads['head_value']/$row_employee_attendance['working_days']*$row_employee_attendance['presented_days'];
											...
											if ($row_heads['head_id'] == 1 || $row_heads['head_id'] == 6 || $row_heads['head_id'] == 7 || $row_heads['head_id'] == 3 || $row_heads['head_id'] == 29) {
											$basic_value = $row_heads['head_value']/$row_employee_attendance['working_days']*$row_employee_attendance['presented_days'];
												$row_heads['head_value']=$basic_value;
											}
											$data_array[$row_heads['head_id']] = $row_heads;
