...
function get_ledger_filter($db, $selected_division) {
    if ($selected_division === 'all') {
        ...
        return "(unit_id IS NULL OR unit_id='' OR unit_id='0' OR visibility='public' OR unit_id='$sd_esc')";
    } else {
        return "1=0";
    }
}
...
if (isset($_GET['ajax']) && $_GET['ajax'] == '1') {
    ...
    if ($type === 'head') {
        ...
        // Child group heads from billit_pl_heads (sub-parents under this head)
...
function node_has_children_under_head($db, $headSno, $selected_division)
{
    ...
    $r = mysqli_query($db, "SELECT COUNT(*) AS cnt FROM billit_pl_heads WHERE parent='$h'");
...
$net_pl_balance = $expense_total - $income_total;

// Net profit/loss to balance
if (abs($net_pl_balance) > 0.001) {
    if ($net_pl_balance < 0) {
        // Profit (Credit > Debit) -> Add Net Profit to Expenses (Debit) side
        $expense_heads[] = [
            'sno' => 'net_profit',
            'description' => '►Profit for the Year ' . date('Y') . '-' . date('y', strtotime('+1 year')) . ' (Group)',
            'total_balance' => abs($net_pl_balance),
            'is_net_node' => true
        ];
        $expense_total += abs($net_pl_balance);
    } else {
        // Loss (Debit > Credit) -> Add Net Loss to Incomes (Credit) side
        $income_heads[] = [
            'sno' => 'net_loss',
        ...
        $income_total += abs($net_pl_balance);
    }
}

/* ── PHP render helper for head rows ── */
function render_heads($heads)
{
    ...
    foreach ($heads as $h) {
        ...
        if (isset($h['is_net_node']) && $h['is_net_node']) {
            $html .= '
    ...
    <div class="col-md-12">
        ...
        <?php if ($selected_division === '' && !$is_sadmin): ?>
            <div class="alert alert-info">
                <i class="fa fa-info-circle mr-2"></i> Please select a division from the dropdown above to view the profit & loss report.
            </div>
        <?php else: ?>
        <div class="bs-wrap">
            ...
            <!-- Header -->
            <div class="bs-header">
                <h5><i class="fa fa-chart-line mr-2"></i>Profit & Loss Account</h5>
                <div>
            ...
            <div class="bs-body">
                ...
                <!-- LEFT: EXPENSES -->
                <div class="bs-side">
                    <div class="side-title">TRADING & P&L EXPENSES</div>
                    <div class="side-content" id="expenseTree">
                        <?php echo render_heads($expense_heads); ?>
                    </div>
                    <div class="side-total">
                        <span>TOTAL</span>
                        <span><?php echo number_format($expense_total, 2); ?></span>
                    </div>
                </div>
                ...
                <!-- RIGHT: INCOMES -->
                <div class="bs-side">
                    <div class="side-title">TRADING & P&L INCOMES</div>
                    <div class="side-content" id="incomeTree">
                        <?php echo render_heads($income_heads); ?>
                    </div>
                    <div class="side-total">
                        <span>TOTAL</span>
                        <span><?php echo number_format($income_total, 2); ?></span>
                    </div>
                </div>
