        ...
        <div class="row">
			...
			<!-- <button onclick="printTable()">Print Table</button> -->
                <div class="card">
					...
					<a target="_blank" href="mpr_report_print.php" ><button type="button" name="student_ledger" class="btn btn-primary"  >Print</button></a>
						...
						if(isset($_POST['report_view'])){
							switch($_POST['report_view']){
								case '1':{
									...
									invoice_civil.project_status_1 as project_status_1, 
									...
									while($row_project = mysqli_fetch_assoc($result_div)){
										...
										if(mysqli_num_rows($invoice_project_status)!=0){
											$invoice_project_status = mysqli_fetch_assoc($invoice_project_status);
										}
										else{
											unset($invoice_project_status);
										...
										if(mysqli_num_rows($result_act)!=0){
											...
											while($row_act = mysqli_fetch_assoc($result_act)){
												$act_txt .= '<tr><td>'.$row_act['activity'].'</td><td>'.$row_act['cummulative_physical_progress'].'</td><td>'.$row_act['remarks'].'</td></tr>';
											}
										}
										else{
											$count_act = 0;
