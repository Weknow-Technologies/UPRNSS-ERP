...
if (isset($_POST['department'])) {
	$expected_keys = ['sub_department_id', 'go_short_number', 'central_share', 'state_share', 'scheme', 'sub_scheme', 'project_type', 'go_type', 'go_number', 'go_date', 'sanction_cost', 'sanction_date', 'project_name_english', 'project_name_hindi', 'project_under', 'add_rows_id'];
	...
	if ($_POST['department'] != "") {
		$sql = ' insert into invoice_new_project (project_type, department,sub_department_id, go_type, go_number, go_date, go_short_number, sanction_cost, sanction_date, central_share, state_share, project_name_english, project_name_hindi, scheme, sub_scheme, project_under, status, created_by, creation_time)
	
    values ("' . $_POST['project_type'] . '", "' . $_POST['department'] . '", "' . $_POST['sub_department_id'] . '", "' . $_POST['go_type'] . '", "' . $_POST['go_number'] . '", "' . $_POST['go_date'] . '", "' . $_POST['go_short_number'] . '", "' . $_POST['sanction_cost'] . '", "' . $_POST['sanction_date'] . '", "' . $_POST['central_share'] . '", "' . $_POST['state_share'] . '", "' . $_POST['project_name_english'] . '", "' . $_POST['project_name_hindi'] . '", "' . $_POST['scheme'] . '", "' . $_POST['sub_scheme'] . '", "' . $_POST['project_under'] . '", "0", "' . $_SESSION['usersno'] . '", "' . date("Y-m-d H:i:s") . '")';
		execute_query($sql);
		if (mysqli_error($db)) {
			...
			for ($i = 1; $i <= $_POST['add_rows_id']; $i++) {
				if (!isset($_POST['project_qun_' . $i]))
					$_POST['project_qun_' . $i] = 1;
				if (!isset($_POST['unit_name_' . $i]))
					$_POST['unit_name_' . $i] = '';
				if (!isset($_POST['district_name_' . $i]))
					$_POST['district_name_' . $i] = '';
				...
				for ($a = 1; $a <= $_POST['project_qun_' . $i]; $a++) {
					...
					values ("' . $inv_id . '","' . $trns_id . '", "' . $_POST['unit_name_' . $i] . '", "' . $_POST['district_name_' . $i] . '", "' . $_POST['project_name_english'] . ' ' . $a . '","' . $_POST['project_name_hindi'] . ' ' . $a . '","' . $_POST['department'] . '", "' . $_POST['sub_department_id'] . '", "' . $_POST['scheme'] . '", "' . $_POST['sub_scheme'] . '", "' . $_POST['project_under'] . '","2", "' . $_SESSION['usersno'] . '", "' . date("Y-m-d H:i:s") . '","0","0","' . $_POST['project_type'] . '")';
						execute_query($sql);
						if (mysqli_error($db)) {
							$msg .= 'Error # 1 : ' . mysqli_error($db) . '>> ' . $sql;
						} else {
						...
						left join uprnss_sub_department on uprnss_sub_department.sno = sub_department_id where uprnss_project_temp.sno="' . $project_id . '" ';
							...
							$sql = 'UPDATE uprnss_project_temp 
									SET erp_code = "' . $serialnum . '"
									WHERE sno = "' . $row['sno'] . '"';
							execute_query($sql);
			...
			if ($msg == '') {
				// header("location: preview_new_project.php?preview=" . urlencode($inv_id));
				$msg .= 'Successfully Added ' . $times . ' New project';
				// exit();
			}
		}
							...
							<div class="col-md-3">
								<div class="form-group">
									...
									<select class="form-control" name="sub_department_id" id="sub_department_id"
										value="<?php echo $_POST['sub_department_id']; ?>"
										tabindex="<?php echo $tab++; ?>">
									</select>
								</div>
							</div>
							<div class="col-md-3">
								<div class="form-group">
									...
									<select name="project_type" id="project_type" class="form-control"
										onchange="onchangetype()" readonly>
										<option value="">--- Select ---</option>
										<option value="1" <?php echo ($_POST['project_type'] == 1 ? 'selected' : ''); ?>>
											शासन
											स्तर</option>
										<option value="2" <?php echo ($_POST['project_type'] == 2 ? 'selected' : ''); ?>>
											जिला
											स्तर </option>
									</select>
								</div>
							...
							<div class="col-md-4 pr-1">
								<div class="form-group">
									...
									<select name="go_type" id="go_type" class="form-control" onchange="onchangetype()">
										<option value="">--Select-- </option>
										<option value="1" <?php echo ($_POST['go_type'] == 1 ? 'selected' : ''); ?>>
											Administrative</option>
										<option value="2" <?php echo ($_POST['go_type'] == 2 ? 'selected' : ''); ?>>
											Financial
										</option>
										<option value="3" <?php echo ($_POST['go_type'] == 3 ? 'selected' : ''); ?>>
						...
						<div class="row" id="admin" style="display:none;">
							<div class="col-md-3 ">
								<div class="form-group">
									...
									<input required type="text" name="go_number" id="go_number" class="form-control"
										placeholder=" " value="<?php echo $_POST['go_number']; ?>" <?php echo $tab++; ?>>
								</div>
							</div>
							<div class="col-md-3 ">
								<div class="form-group">
									...
									<script type="text/javascript" language="javascript">
										document.writeln(DateInput('go_date', 'user_form', true, 'YYYY-MM-DD', '<?php echo date("Y-m-d"); ?>', <?php echo $tab++; ?>));
									</script>
								</div>
							</div>
							<!--<div class="col-md-3 ">
						...
						<div class="row" id="financial" style="display:none;">
							...
							<div class="col-md-3 ">
								<div class="form-group">
									...
									<script type="text/javascript" language="javascript">
										document.writeln(DateInput('sanction_date', 'user_form', true, 'YYYY-MM-DD', '<?php echo date("Y-m-d"); ?>', <?php echo $tab++; ?>));
									</script>
								</div>
							</div>
							<!--<div class="col-md-3 ">
		...
		<div class="row">
			<div class="col-md-12">
				<div class="card">
					...
					<div class="card-body border rounded m-3 p-2 border-secondary">
						<div class="row ">
							<div class="col-md-6 ">
								<div class="form-group">
									<label>Project Name (English)</label>
									<input type="text" name="project_name_english" id="project_name_english"
										class="form-control" placeholder=""
										value="<?php echo $_POST['project_name_english']; ?>"
										tabindex="<?php echo $tab++; ?>">
									</input>
								</div>
							</div>
							<div class="col-md-6 ">
								<div class="form-group">
									...
									<input required type="text" name="project_name_hindi" id="project_name_hindi"
										class="form-control" placeholder=" "
										value="<?php echo $_POST['project_name_hindi']; ?>"
										tabindex="<?php echo $tab++; ?>">
									</input>
								</div>
							</div>
						...
						<div class="row">
							<div class="col-md-4">
								<div class="form-group">
									...
									<select class="form-control" name="scheme" id="scheme"
										tabindex="<?php echo $tab++; ?>" onChange="fill_sub_scheme(this.value)">
							...
							<div class="col-md-4">
								<div class="form-group">
									...
									<select class="form-control" name="sub_scheme" id="sub_scheme"
							...
							<div class="col-md-4">
								<div class="form-group">
									...
									<select class="form-control" name="project_under" id="project_under"
										tabindex="<?php echo $tab++; ?>">
										<option value="">--Select-- </option>
										<option value="Governance level" <?php echo ($_POST['project_under'] == 'Governance level' ? 'selected' : ''); ?>>
											Governance
											level</option>
								...
								<div id="add_rows_length" class="row">
									<div class="col-md-4 "><?php echo $i; ?>.
										<div class="form-group">
											...
											<select required class="form-control" name="unit_name_<?php echo $i; ?>"
												id="unit_name_<?php echo $i; ?>" tabindex="<?php echo $tab++; ?>">
												<option value="">--- Select ---</option>
												<?php
												...
												while ($data = mysqli_fetch_array($run)) {
													...
													if (isset($_POST['unit_name' . $i])) {
														if ($_POST['unit_name' . $i] == $data['s_no']) {
															echo ' selected="Selected"';
														}
													}
									...
									<div class="col-md-4 ">
										<div class="form-group">
											...
											$run = mysqli_query($db, $query);
											?>
											<select required class="form-control" name="district_name_<?php echo $i; ?>"
												id="district_name_<?php echo $i; ?>">
												<option value="">--- Select ---</option>
												<?php
												while ($data = mysqli_fetch_array($run)) {
													echo "<option value='$data[sno]'>$data[district_name_english]</option>";
												}
									...
									<div class="col-md-3 ">
										<div class="form-group">
											<label>project Quantity (परियोजनाओ की संख्या)</label>
											<input required type="text" name="project_qun_<?php echo $i; ?>"
												id="project_qun_<?php echo $i; ?>" class="form-control" placeholder=" "
												value="<?php echo $_POST['project_qun_' . $i]; ?>"
												tabindex="<?php echo $tab++; ?>">
										</div>
									</div>
									<div class="col-md-1 d-flex justify-content- align-items-center">
										<button type="button" id="add_button" class="btn btn-info pull-right"
											onClick="add_rows()">Add</button>
						...
						<div class="col-md-12 pr-1" align="center">
							<div class="form-group">
								<button type="submit" name="submit"
									class="btn btn-success btn-fill pull-right">Submit</button>
				...
				<table class="table table-hover table-striped table-bordered" id="reportTable">
					...
					<tbody>
						...
						if ($_SESSION['usertype'] == "1") {
							$sql = 'SELECT * FROM `invoice_new_project` where status!=5 and created_by="' . $_SESSION['usersno'] . '"';
						} else {
						...
						while ($row = mysqli_fetch_assoc($result)) {
							...
							$department_res = execute_query($sql);
							$department = $department_res ? mysqli_fetch_assoc($department_res) : null;
							if (!$department)
							...
							$act_txt = '<table class="table table-bordered table-sm mb-0">
							...
							if (mysqli_num_rows($result_act) != 0) {
								...
								while ($row_act = mysqli_fetch_assoc($result_act)) {
									...
									$division_res = execute_query($sql);
									$division = $division_res ? mysqli_fetch_assoc($division_res) : null;
									if (!$division)
										$division = ['division_name' => ''];

									$sql = 'select * from uprnss_district where sno="' . $row_act['district_id'] . '"';
									$district_res = execute_query($sql);
									$district = $district_res ? mysqli_fetch_assoc($district_res) : null;
									if (!$district)
...
<script>
	...
	function fill_sub_scheme(val, selected = '') {
		var data = { "term": "b", "id": "sub_scheme", "val": val };
		...
		$.ajax({
			...
			success: function (ajaxdata) {
				...
				$.each(ajaxdata, function (key, value) {
					...
					txt += '>' + value.sub_scheme_hindi + '</option>';
