...
if(isset($_POST['submit'])){
	if($_POST['edit_sno']==''){
		$sql = 'insert into dp_subgroup (subgroup_name) values ("'.$_POST['subgroup_name'].'")';
		execute_query($sql);
		if(mysqli_error($db)){ 
...
if(isset($_GET['edit_sno'])){
	$sql = 'select * from dp_subgroup where sno="'.$_GET['edit_sno'].'"';
	$data = mysqli_fetch_assoc(execute_query($sql));
	$_POST['edit_sno'] = $data['sno'];
	$_POST['subgroup_name'] = $data['subgroup_name'];
}	

if(isset($_GET['del'])){
	$sql = 'delete from dp_subgroup where sno="'.$_GET['del'].'"';
	execute_query($sql);
						...
						<div class="row" id="">
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label >Employee Sub-Group</label><br>
                                    <input type="text" name="subgroup_name" id="subgroup_name" class="form-control" placeholder="" value="<?php echo $_POST['subgroup_name']; ?>" tabindex="<?php echo $tab++; ?>">
                                </div>
                            </div>
                        </div>
						<div id="test"></div>
